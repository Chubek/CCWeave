...
error_is_incomplete (const moonix_state *state)
{
  ...
  return message_len >= marker_len
         && strcmp (message + message_len - marker_len, MOONIX_REPL_EOF_MARKER)
                == 0;
}
...
load_expression (moonix_state *state, const char *source, size_t source_len)
{
  ...
  status = moonix_load_buffer (state, expression.s, expression.l, "=stdin");
  free (expression.s);
  return status;
}
  ...
  for (;;)
    {
      ...
      while (!done)
        {
          const char *prompt = first_line ? MOONIX_REPL_PROMPT
