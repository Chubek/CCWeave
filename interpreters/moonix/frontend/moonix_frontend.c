...
static const unsigned char magic[8] = {'M','O','O','N','I','X','B','C'};

int moonix_source_has_close_attribute(const char *source, size_t n)
{
    for (size_t i=0;i+6<n;i++)
        if (source[i]=='<' && !memcmp(source+i, "<close>", 7)) return 1;
    return 0;
}

int moonix_source_has_goto(const char *source, size_t n)
{
    for (size_t i=0;i+4<n;i++)
        if (!memcmp(source+i, "goto", 4) &&
            (i==0 || (source[i-1]<'a'||source[i-1]>'z')) &&
            (i+4==n || (source[i+4]<'a'||source[i+4]>'z'))) return 1;
    return 0;
}

static int moonix_source_is_incomplete(const char *source, size_t n)
{
    int depth = 0;
    for (size_t i = 0; i + 1 < n; ++i) {
        if ((i == 0 || !((source[i - 1] >= 'a' && source[i - 1] <= 'z') ||
                         (source[i - 1] >= 'A' && source[i - 1] <= 'Z') ||
                         source[i - 1] == '_')) &&
            source[i] == 'd' && source[i + 1] == 'o' &&
            (i + 2 == n || !((source[i + 2] >= 'a' && source[i + 2] <= 'z') ||
...
moonix_status moonix_frontend_compile(moonix_state *state, const char *source,
                                      size_t source_len, const char *chunk_name,
                                      moonix_chunk *chunk)
{
    (void)chunk_name;
    if (!state || !source || !chunk) return MOONIX_ERR_ARGUMENT;
    memset(chunk, 0, sizeof(*chunk));
    if (moonix_source_has_close_attribute(source, source_len)) {
        moonix_set_error(state, "to-be-closed variables are not supported in Moonix v0.1");
    ...
    if (!chunk->data) return MOONIX_ERR_OOM;
    ...
    for (size_t i=0;i<source_len;i++) if (source[i]=='\n') chunk->source_line_count++;
