...
static void
check_plan (const char *name)
{
  const char *expected_hash = strcmp (name, "T1") == 0
                                  ? "a967076f76b9148412e9f5068088e4ce65fb848cc"
                                    "d0709dddf2521cd5ab3503d"
                                  : "804695292da328a7e1a7afdd89a9761b5e5c9d3c5"
                                    "d1e335804e6f3c362a1f895";
  char path[512];
  ...
  CCW_CHECK (strcmp (hash1, expected_hash) == 0,
             "%s plan hash drifted: expected %s, got %s", name, expected_hash,
             hash1);
