...
static int edge_present(const char *text, unsigned from, unsigned to)
{
    char needle[64];
...
static int validate_on1x_plan(const ccw_plan *plan, char *error, size_t capacity)
{
    ...
    for (line = strtok(copy, "\n"); line != NULL; line = strtok(NULL, "\n")) {
        ...
        if (sscanf(line, "node %u %u %127s %511[^\n]",
                   &node.id, &node.kind, node.name, node.rest) < 3)
            continue;
        ...
        if (node.kind == 1 && strstr(node.rest, "vm.") != NULL) {
            ...
            if (strstr(node.rest, "vm.inline-cache") != NULL) has_ic = 1;
    ...
    for (size_t i = 0; i < vm_count; ++i) {
        if (vm_nodes[i] >= barrier ||
            !edge_present(text, vm_nodes[i], barrier)) {
            snprintf(error, capacity,
...
static moonix_status load_plan(moonix_state *state, moonix_tier tier)
{
    int index = (int)tier - (int)MOONIX_TIER_T1;
    char path[1024];
    ccw_sched_error sched_error = {0};
    ccw_plan *plan = NULL;
    if (state->plans[index] != NULL) return MOONIX_OK;
