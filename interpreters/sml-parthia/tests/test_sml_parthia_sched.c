...
static unsigned
find_node (const char *text, const char *name)
{
...
check_level (const char *level)
{
  ...
  CCW_CHECK (ccw_sml_parthia_load_plan (level, CCW_SML_PARTHIA_MANIFEST_DIR,
                                        CCW_SML_PARTHIA_SCHED_DIR, &plan,
                                        &error),
  ...
  if (regalloc == 0)
    regalloc = find_node (text, "regalloc-linear");
  if (regalloc == 0)
    regalloc = find_node (text, "regalloc-graph-color");
  codegen = find_node (text, "codegen-x86-64");

  CCW_CHECK (
      pattern && closure && lift && tail && exceptions && gc && safepoint
          && barrier && tree && legalize && schedule && regalloc && codegen,
      "%s plan is missing required functional/runtime/backend stages", level);
  if (pattern && closure && lift && tail && exceptions && gc && safepoint
      && barrier && tree && legalize && schedule && regalloc && codegen)
    {
      CCW_CHECK (
          has_edge (text, pattern, pipeline)
  ...
  if (strcmp (level, "O2") == 0)
    {
      CCW_CHECK (
          strstr (text, "vec-width analysis.vector-width") != NULL
