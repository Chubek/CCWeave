...
static char *repl_strdup(const char *text)
{
    size_t length;
    char *copy;
    if (!text) return NULL;
...
static char *join_path(const char *root, const char *name)
{
    size_t n;
    char *path;
    if (!root || !*root || !name || !*name) return NULL;
...
static char *resolve_library(const char *name)
{
    ...
    while (*cursor) {
        ...
        int variant_count = 1;
        if (!root) return NULL;
        memcpy(root, cursor, length); root[length] = '\0';
        snprintf(variants[0], sizeof(variants[0]), "%s", name);
        if (!strstr(name, ".so")) {
            snprintf(variants[variant_count++], sizeof(variants[0]), "%s.so", name);
            snprintf(variants[variant_count++], sizeof(variants[0]), "lib%s.so", name);
            snprintf(variants[variant_count++], sizeof(variants[0]), "lib%s", name);
        }
        for (int variant = 0; variant < variant_count; variant++) {
            candidate = join_path(root, variants[variant]);
            if (candidate) {
                FILE *file = fopen(candidate, "rb");
                if (file) { fclose(file); free(root); return candidate; }
                free(candidate);
            }
        }
        free(root);
...
static int print_module_signatures(const char *session, const char *module)
{
    const char *cursor = session;
    size_t module_len = module ? strlen(module) : 0;
    int found = 0;
    if (!session || !module) return 0;
    while (*cursor) {
    ...
    if (!found) fprintf(stderr, "sml-parthia: module %s is not open\n", module);
    return found;
}

static int handle_directive(const char *line, ccw_sml_parthia_runtime *runtime,
                            char **session, size_t *session_len)
{
    ...
    if (strcmp(command, "use") == 0) {
        ...
        fputs(ccw_sml_parthia_core_ast(program), stdout); fputc('\n', stdout);
        ccw_sml_parthia_program_destroy(program); free(error); free(path);
        return 1;
    }
                           ...
                           char **session, size_t *session_len)
{
    ...
    for (;;) {
        ...
        if (!run_phrase(*source, phrase_length, runtime)) status = 0;
        else {
            size_t old_len = *session_len;
            char *grown = (char *)realloc(*session, old_len + phrase_length + 2u);
            if (!grown) status = 0;
            else {
                *session = grown;
...
int sml_parthia_repl(void)
{
    ...
    for (;;) {
        ...
        if (source_len == 0 && line[0] == '#') {
            int directive_status = handle_directive(line, runtime,
                                                    &session, &session_len);
            free(line);
            if (directive_status < 0) {
                free(source); free(session);
                linenoiseHistoryFree();
                ccw_sml_parthia_runtime_free(runtime);
                return status;
            }
            if (!directive_status) status = 1;
            continue;
        }
        ...
        free(line);
        if (!process_phrases(&source, &source_len, runtime,
                             &session, &session_len)) status = 1;
    }
}
