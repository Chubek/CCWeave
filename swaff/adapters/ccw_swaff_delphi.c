...
static char *dup_string(const char *s)
{
    ...
    if (s == NULL || kputs(s, &out) == EOF) return NULL;
    return ks_release(&out);
}

static void set_error(char **out, const char *message)
{
    if (out != NULL) *out = dup_string(message);
}
...
static bool is_node(TSNode n, const char *type)
{
...
static char *text(delphi_ctx *ctx, TSNode n)
{
    uint32_t a, b;
    kstring_t out = { 0, 0, NULL };
    ...
    if (b < a || (size_t)b > ctx->source_len) return NULL;
    if (kputsn(ctx->source + a, (int)(b - a), &out) == EOF) return NULL;
    return ks_release(&out);
}

static void fail(delphi_ctx *ctx, const char *message)
{
    if (!ctx->failed) {
        ctx->failed = true;
...
static bool scan_errors(TSNode n, ccw_swaff_report *report)
{
    bool found = false;
    ...
    for (uint32_t i = 0; i < ts_node_child_count(n); i++)
        if (scan_errors(ts_node_child(n, i), report)) found = true;
    return found;
}

static bool malformed(delphi_ctx *ctx, TSNode n)
{
    if (!ts_node_is_error(n) && !ts_node_is_missing(n) &&
        !is_node(n, "ERROR"))
        return false;
    if (ctx->policy == CCW_SWAFF_REJECT_ON_ERROR) ctx->rejected = true;
    else ctx->report->recovered_subtrees++;
    return true;
}

static char *new_temp(delphi_ctx *ctx)
{
    char b[40];
...
static char *new_block(delphi_ctx *ctx, const char *stem)
{
    char b[48];
...
static void clear_locals(delphi_ctx *ctx)
{
...
static bool add_local(delphi_ctx *ctx, const char *name)
{
...
static bool is_local(const delphi_ctx *ctx, const char *name)
{
...
static ccw_ir_type type_from_text(delphi_ctx *ctx, TSNode n)
{
    char *s = text(ctx, n);
    ...
    if (strstr(s, "void") || strstr(s, "procedure")) type = CCW_TY_VOID;
    else if (strstr(s, "single") || strstr(s, "real")) type = CCW_TY_F32;
    else if (strstr(s, "double") || strstr(s, "extended")) type = CCW_TY_F64;
    else if (strstr(s, "boolean")) type = CCW_TY_I1;
    else if (strstr(s, "byte") || strstr(s, "shortint") || strstr(s, "char"))
        type = CCW_TY_I8;
    else if (strstr(s, "word") || strstr(s, "smallint")) type = CCW_TY_I16;
    else if (strstr(s, "integer") || strstr(s, "longint")) type = CCW_TY_I32;
    else if (strchr(s, '^') || strstr(s, "pointer")) type = CCW_TY_PTR;
    free(s);
    return type;
}

static bool terminated(const ccw_ir *ir, ccw_node block)
{
    int n = ccw_ir_block_instr_count(ir, block);
    if (!n) return false;
    const char *op = ccw_ir_instr_opcode(
        ir, ccw_ir_block_instr_ref(ir, block, n - 1));
    return op && (!strcmp(op, "ret") || !strcmp(op, "br") ||
                  !strcmp(op, "br.cond"));
}

static char *lower_expr(delphi_ctx *ctx, ccw_node block, TSNode n);

static char *lower_identifier(delphi_ctx *ctx, ccw_node block, TSNode n)
{
    ...
    if (dest == NULL || ccw_kliche_local_load(ctx->ir, block, dest, name,
                                               CCW_TY_I64) == 0) {
        free(dest);
...
static char *lower_number(delphi_ctx *ctx, ccw_node block, TSNode n)
{
    char *s = text(ctx, n);
    char *end = NULL;
    int base = 10;
    ...
    if (s[0] == '$') { memmove(s, s + 1, strlen(s)); base = 16; }
    errno = 0;
...
{
    static const struct { const char *src; const char *ir; } map[] = {
        { "+", "iadd" }, { "-", "isub" }, { "*", "imul" },
    ...
    for (size_t i = 0; i < sizeof(map) / sizeof(map[0]); i++)
        if (!strcmp(op, map[i].src)) return map[i].ir;
    return NULL;
}
...
{
    if (ts_node_is_null(n)) return NULL;
    if (malformed(ctx, n)) return NULL;
    if (is_node(n, "identifier")) return lower_identifier(ctx, block, n);
    if (is_node(n, "literalNumber")) return lower_number(ctx, block, n);
    if (is_node(n, "kTrue") || is_node(n, "kFalse")) {
        char *d = new_temp(ctx);
        if (d == NULL || ccw_kliche_int_const(ctx->ir, block, d,
                                               is_node(n, "kTrue")) == 0) {
            free(d); fail(ctx, "swaff Delphi: could not lower boolean literal");
            return NULL;
        }
        return d;
    }
    if (is_node(n, "exprParens")) return lower_expr(ctx, block, first_named(n));
    if (is_node(n, "exprUnary")) {
        ...
        const char *ir_op = op && !strcmp(op, "not") ? "inot" :
                            op && !strcmp(op, "-") ? "ineg" : NULL;
        free(op);
        if (!arg || !ir_op || !dest ||
            ccw_kliche_unary(ctx->ir, block, ir_op, dest, arg, CCW_TY_I64) == 0) {
            free(arg); free(dest); fail(ctx, "swaff Delphi: unsupported unary expression");
            return NULL;
        }
        free(arg);
        return dest;
    ...
    if (is_node(n, "exprCall")) {
        ...
        if (!ts_node_is_null(args)) {
            ...
            for (uint32_t i = 0; i < count && argc < CCW_DELPHI_MAX_ARGS; i++)
                argv[argc++] = lower_expr(ctx, block, ts_node_named_child(args, i));
        }
        char *dest = new_temp(ctx);
        ccw_node call = 0;
        if (callee && dest)
        ...
        free(callee);
        if (!call || ctx->failed) {
            free(dest); fail(ctx, "swaff Delphi: could not lower call");
            return NULL;
        }
        return dest;
    }
...
static void lower_body(delphi_ctx *ctx, ccw_node *block, TSNode body)
{
    if (ts_node_is_null(body)) return;
    for (uint32_t i = 0; i < ts_node_named_child_count(body) &&
                         !ctx->failed && !ctx->rejected; i++)
        lower_statement(ctx, block, ts_node_named_child(body, i));
}

static void lower_statement(delphi_ctx *ctx, ccw_node *block, TSNode n)
{
    if (malformed(ctx, n)) return;
    if (is_node(n, "block") || is_node(n, "blockTr") ||
        ...
        if (!lhs || !rhs) {
            fail(ctx, "swaff Delphi: malformed assignment");
                   ...
                   (ctx->fn && ccw_ir_function_name(ctx->ir, ctx->fn) &&
                    strcmp(lhs, ccw_ir_function_name(ctx->ir, ctx->fn)) == 0)) {
            free(ctx->result_value);
            ctx->result_value = dup_string(rhs);
            if (ctx->result_value == NULL) fail(ctx, "swaff Delphi: out of memory");
        } else {
        ...
        if (!cond || !tb || !eb || !mb ||
            ...
            if (!terminated(ctx->ir, then_block)) ccw_kliche_jump(ctx->ir, then_block, mb);
            if (is_node(n, "ifElse")) lower_statement(ctx, &else_block, field(n, "else"));
            if (!terminated(ctx->ir, else_block)) ccw_kliche_jump(ctx->ir, else_block, mb);
            *block = merge_block;
        }
        free(cond); free(tb); free(eb); free(mb);
        ...
        ccw_node b = body_name ? ccw_ir_block_add(ctx->ir, ctx->fn, body_name) : 0;
        ccw_node x = exit ? ccw_ir_block_add(ctx->ir, ctx->fn, exit) : 0;
        char *cond = lower_expr(ctx, *block, field(n, "condition"));
        if (!head || !body_name || !exit || !cond) fail(ctx, "swaff Delphi: could not construct while loop");
        else {
            ccw_kliche_jump(ctx->ir, *block, head);
            ccw_kliche_branch_if(ctx->ir, h, cond, body_name, exit);
            lower_statement(ctx, &b, field(n, "body"));
            if (!terminated(ctx->ir, b)) ccw_kliche_jump(ctx->ir, b, head);
            *block = x;
        }
        free(cond); free(head); free(body_name); free(exit);
    } else if (is_node(n, "raise") || is_node(n, "try") || is_node(n, "case") ||
               is_node(n, "for") || is_node(n, "foreach") || is_node(n, "with") ||
               is_node(n, "repeat") || is_node(n, "goto") || is_node(n, "asm")) {
...
static void lower_local_decls(delphi_ctx *ctx, TSNode node, ccw_node block)
{
...
static void lower_proc(delphi_ctx *ctx, TSNode def)
{
    TSNode header = field(def, "header");
    char *name = text(ctx, field(header, "name"));
    TSNode result = field(header, "type");
    ccw_ir_type result_type = ts_node_is_null(result) ? CCW_TY_VOID
                                                       : type_from_text(ctx, result);
    if (!name) { fail(ctx, "swaff Delphi: procedure has no name"); return; }
    ctx->fn = ccw_ir_function_add(ctx->ir, name, result_type);
    free(name);
    if (!ctx->fn) { fail(ctx, "swaff Delphi: could not create procedure"); return; }
    ctx->temp_index = ctx->block_index = 0;
    clear_locals(ctx);
    ctx->result_name = result_type == CCW_TY_VOID ? NULL : dup_string(
        ccw_ir_function_name(ctx->ir, ctx->fn));
    ...
    if (ts_node_is_null(local)) lower_local_decls(ctx, def, block);
...
ccw_ir *ccw_swaff_lower_delphi(const ccw_swaff_frontend *fe,
                               ...
                               ccw_swaff_report *report, char **error_message)
{
    ...
    if (fe != &g_frontend_delphi || !source || !module_name ||
        ...
        set_error(error_message, "swaff Delphi: invalid frontend, source, or module name");
        return NULL;
    }
    TSParser *parser = ts_parser_new();
    if (!parser || !ts_parser_set_language(parser, tree_sitter_pascal())) {
        if (parser) ts_parser_delete(parser);
        set_error(error_message, "swaff Delphi: vendored Pascal grammar is ABI-incompatible");
        return NULL;
    }
    TSTree *tree = ts_parser_parse_string(parser, NULL, source, (uint32_t)source_len);
    if (!tree) {
        ts_parser_delete(parser);
        set_error(error_message, "swaff Delphi: parser produced no syntax tree");
        return NULL;
    }
    ...
    for (uint32_t i = 0; i < ts_node_named_child_count(root) &&
                         !ctx.failed && !ctx.rejected; i++) {
        TSNode child = ts_node_named_child(root, i);
        if (malformed(&ctx, child)) continue;
        if (is_node(child, "defProc")) lower_proc(&ctx, child);
            ...
            for (uint32_t j = 0; j < ts_node_named_child_count(child); j++) {
                ...
                if (is_node(d, "defProc")) lower_proc(&ctx, d);
            }
        } else if (!is_node(child, "declVars") && !is_node(child, "declTypes") &&
                   !is_node(child, "declConsts") && !is_node(child, "declUses") &&
                   !is_node(child, "moduleName") && !is_node(child, "kProgram") &&
                   !is_node(child, "kEndDot")) {
