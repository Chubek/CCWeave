...
static char *ocaml_strdup(const char *s)
{
...
static void ocaml_set_error(char **error_message, const char *message)
{
    if (error_message != NULL) *error_message = ocaml_strdup(message);
}
...
static char *node_text(TSNode node, const char *source, size_t source_len)
{
    ...
    if (kputsn(source + start, (int)n, &text) == EOF) return NULL;
    return ks_release(&text);
}

static void lower_fail(ccw_ocaml_lower *ctx, const char *message)
{
...
static char *new_temp(ccw_ocaml_lower *ctx)
{
    char name[40];
...
static char *new_block_name(ccw_ocaml_lower *ctx, const char *stem)
{
    char name[48];
...
static void clear_function_names(ccw_ocaml_lower *ctx)
{
...
static bool add_parameter_name(ccw_ocaml_lower *ctx, const char *name)
{
...
static bool is_parameter(const ccw_ocaml_lower *ctx, const char *name)
{
    for (int i = 0; i < ctx->parameter_count; i++)
...
static const char *lookup_alias(const ccw_ocaml_lower *ctx, const char *name)
{
...
static bool push_alias(ccw_ocaml_lower *ctx, const char *name, const char *reg)
{
...
static void pop_aliases(ccw_ocaml_lower *ctx, int saved_count)
{
...
static char *lower_number(ccw_ocaml_lower *ctx, ccw_node block, TSNode node)
{
    ...
    for (size_t i = 0; i < length; i++)
        if (text[i] != '_') normalized[out++] = text[i];
    if (out > 0 && (normalized[out - 1] == 'l' ||
                    normalized[out - 1] == 'L' ||
                    normalized[out - 1] == 'n'))
        out--;
    normalized[out] = '\0';
...
static char *lower_boolean(ccw_ocaml_lower *ctx, ccw_node block, TSNode node)
{
    char *text = node_text(node, ctx->source, ctx->source_len);
...
static char *lower_value_path(ccw_ocaml_lower *ctx, TSNode node)
{
...
{
    struct operator_map { const char *ocaml; const char *ir; };
    static const struct operator_map operators[] = {
        { "+", "iadd" }, { "-", "isub" }, { "*", "imul" },
        { "/", "idiv" }, { "mod", "irem" },
        { "land", "iand" }, { "lor", "ior" }, { "lxor", "ixor" },
...
static char *lower_binary(ccw_ocaml_lower *ctx, ccw_node *block, TSNode node)
{
    char *operator_text =
    ...
    if (left == NULL || right == NULL || dest == NULL ||
        ccw_kliche_binary(ctx->ir, *block, opcode, dest, left, right, type) == 0) {
        free(left);
        free(right);
        free(dest);
        free(operator_text);
...
static char *lower_unary(ccw_ocaml_lower *ctx, ccw_node *block, TSNode node)
{
    char *operator_text =
        node_text(field(node, "operator"), ctx->source, ctx->source_len);
    ...
    if (operand == NULL || dest == NULL ||
        ccw_kliche_unary(ctx->ir, *block, opcode, dest, operand,
                         strcmp(opcode, "logic.not") == 0
                             ? CCW_TY_I1 : CCW_TY_I64) == 0) {
        free(operand);
        free(dest);
        free(operator_text);
        if (!ctx->failed)
...
static int collect_arguments(ccw_ocaml_lower *ctx, ccw_node *block, TSNode node,
                             char **owned, const char **arguments)
{
    ...
    for (uint32_t i = 0; i < children; i++) {
        ...
        if (owned[count] == NULL) return count;
...
static char *lower_application(ccw_ocaml_lower *ctx, ccw_node *block,
                               TSNode node)
{
    ...
    if (!node_is(function_node, "value_path")) {
        ctx->report->unsupported_nodes++;
        lower_fail(ctx,
                   "swaff OCaml: only named or parameter function application is supported");
        return NULL;
    }
    char *function_name =
    ...
    int argument_count =
        collect_arguments(ctx, block, node, owned, arguments);
    char *result = NULL;
...
static char *lower_if(ccw_ocaml_lower *ctx, ccw_node *block, TSNode node)
{
    ...
    for (uint32_t i = 0; i < child_count; i++) {
        ...
        if (node_is(child, "then_clause")) then_clause = child;
        else if (node_is(child, "else_clause")) else_clause = child;
    }

    char *then_value = ctx->failed ? NULL :
        lower_expression(ctx, &then_block, field(then_clause, "expression"));
    if (!ctx->failed &&
        (then_value == NULL ||
    ...
    if (!ctx->failed && !ts_node_is_null(else_clause))
        else_value =
            lower_expression(ctx, &else_block, field(else_clause, "expression"));
    else if (!ctx->failed)
...
static char *lower_sequence(ccw_ocaml_lower *ctx, ccw_node *block, TSNode node)
{
    char *result = NULL;
...
static char *lower_let_expression(ccw_ocaml_lower *ctx, ccw_node *block,
                                  TSNode node)
{
    ...
    if (binding_count != 1) {
        ctx->report->unsupported_nodes++;
        lower_fail(ctx,
                   "swaff OCaml: local let-and bindings are unsupported");
        return NULL;
    }
...
static bool add_function_parameter(ccw_ocaml_lower *ctx, TSNode parameter)
{
...
static void lower_function_binding(ccw_ocaml_lower *ctx, TSNode binding)
{
    TSNode name_node = simple_pattern_name(field(binding, "pattern"));
    if (ts_node_is_null(name_node)) {
        ctx->report->unsupported_nodes++;
...
static void lower_value_definition(ccw_ocaml_lower *ctx, TSNode definition)
{
    uint32_t count = ts_node_named_child_count(definition);
    for (uint32_t i = 0; i < count && !ctx->failed; i++) {
        TSNode child = ts_node_named_child(definition, i);
