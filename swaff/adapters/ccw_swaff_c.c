...
{
    if (error_message != NULL) *error_message = ccw_strdup(msg);
}
...
static void lower_fail(ccw_c_lower *ctx, const char *msg)
{
...
{
    if (ts_node_is_null(node)) return null_node();
...
{
    if (node_is(node, type)) return node;
    uint32_t n = ts_node_named_child_count(node);
    for (uint32_t i = 0; i < n; i++) {
...
static bool malformed_node(ccw_c_lower *ctx, TSNode node)
{
...
static ccw_ir_type lower_type(TSNode node, const char *source, size_t source_len)
{
    char *text = node_text(node, source, source_len);
    if (text == NULL) return CCW_TY_I64;
    ccw_ir_type type = CCW_TY_I64;
    if (strstr(text, "void") != NULL) type = CCW_TY_VOID;
    else if (strstr(text, "double") != NULL) type = CCW_TY_F64;
    else if (strstr(text, "float") != NULL) type = CCW_TY_F32;
    else if (strstr(text, "char") != NULL) type = CCW_TY_I8;
    else if (strstr(text, "short") != NULL) type = CCW_TY_I16;
    else if (strchr(text, '*') != NULL) type = CCW_TY_PTR;
    free(text);
    return type;
}

static bool is_local(const ccw_c_lower *ctx, const char *name)
{
...
static bool add_local(ccw_c_lower *ctx, const char *name)
{
    if (is_local(ctx, name)) return true;
...
static void clear_locals(ccw_c_lower *ctx)
{
    for (int i = 0; i < ctx->local_count; i++) free(ctx->locals[i]);
    ctx->local_count = 0;
}

static char *new_temp(ccw_c_lower *ctx)
{
    char name[32];
...
static char *new_block_name(ccw_c_lower *ctx, const char *stem)
{
    char name[48];
...
static void lower_statement(ccw_c_lower *ctx, ccw_node *block, TSNode statement);

static char *lower_identifier(ccw_c_lower *ctx, ccw_node block, TSNode expr)
{
    ...
    if (!is_local(ctx, name)) return name; /* function parameter */
...
static char *lower_number(ccw_c_lower *ctx, ccw_node block, TSNode expr)
{
...
{
    struct op_map { const char *c; const char *ir; };
    ...
    for (size_t i = 0; i < sizeof(map) / sizeof(map[0]); i++)
        if (strcmp(operator_text, map[i].c) == 0) return map[i].ir;
    return NULL;
}

static char *lower_binary(ccw_c_lower *ctx, ccw_node block, TSNode expr)
{
    ...
    if (lhs == NULL || rhs == NULL || dest == NULL ||
        ...
        if (!ctx->failed) lower_fail(ctx, "swaff C: could not lower binary expression");
        return NULL;
    }
    free(lhs);
    free(rhs);
...
static char *lower_unary(ccw_c_lower *ctx, ccw_node block, TSNode expr)
{
    TSNode op = field(expr, "operator");
    ...
    if (op_text != NULL) {
        if (strcmp(op_text, "-") == 0) opcode = "ineg";
        else if (strcmp(op_text, "~") == 0) opcode = "inot";
        else if (strcmp(op_text, "!") == 0) opcode = "logic.not";
        else if (strcmp(op_text, "+") == 0) opcode = "imov";
    }
    if (opcode == NULL) {
        free(op_text);
    ...
    if (operand == NULL || dest == NULL ||
        ccw_kliche_unary(ctx->ir, block, opcode, dest, operand,
                         strcmp(opcode, "logic.not") == 0 ? CCW_TY_I1
                                                          : CCW_TY_I64) == 0) {
        free(operand); free(dest); free(op_text);
        if (!ctx->failed) lower_fail(ctx, "swaff C: could not lower unary expression");
        return NULL;
    }
    free(operand);
    free(op_text);
...
static char *lower_call(ccw_c_lower *ctx, ccw_node block, TSNode expr)
{
    ...
    if (callee == NULL || dest == NULL ||
        ccw_kliche_call(ctx->ir, block, dest, callee, args, arg_count,
                        CCW_TY_I64) == 0) {
        if (!ctx->failed) lower_fail(ctx, "swaff C: could not lower function call");
        free(dest);
        dest = NULL;
    }
...
static char *lower_assignment(ccw_c_lower *ctx, ccw_node block, TSNode expr)
{
    TSNode left = field(expr, "left");
    TSNode right = field(expr, "right");
    TSNode op = field(expr, "operator");
    ...
    if (op_text != NULL && strcmp(op_text, "=") != 0) {
        ...
        if (op_text[0] == '<' || op_text[0] == '>') basic_op[1] = op_text[1];
        ...
        if (opcode == NULL || current == NULL || rhs == NULL || value == NULL ||
            ccw_kliche_binary(ctx->ir, block, opcode, value, current, rhs,
                              CCW_TY_I64) == 0) {
            free(current); free(rhs); free(value); free(name); free(op_text);
            if (!ctx->failed) lower_fail(ctx, "swaff C: could not lower compound assignment");
            return NULL;
        }
        free(current);
        free(rhs);
    ...
    if (value == NULL ||
        ...
        free(value); free(name); free(op_text);
        if (!ctx->failed) lower_fail(ctx, "swaff C: could not lower assignment");
        return NULL;
    }
    free(name);
    free(op_text);
...
static void lower_declarator(ccw_c_lower *ctx, ccw_node block, TSNode declarator,
                             ccw_ir_type type)
{
    ...
    if (name == NULL || !add_local(ctx, name) ||
        ...
        free(name);
        if (!ctx->failed) lower_fail(ctx, "swaff C: could not lower local declaration");
        return;
    }
...
static void lower_declaration(ccw_c_lower *ctx, ccw_node block, TSNode declaration)
{
    ccw_ir_type type =
...
static void lower_return(ccw_c_lower *ctx, ccw_node block, TSNode statement)
{
...
static void lower_if(ccw_c_lower *ctx, ccw_node *block, TSNode statement)
{
    ...
    if (condition == NULL || then_name == NULL || else_name == NULL ||
        merge_name == NULL) {
        free(condition); free(then_name); free(else_name); free(merge_name);
        if (!ctx->failed) lower_fail(ctx, "swaff C: could not construct if blocks");
        return;
    }
    ...
    if (then_block == 0 || else_block == 0 || merge_block == 0 ||
        ccw_kliche_branch_if(ctx->ir, *block, condition, then_name, else_name) == 0) {
...
static void lower_compound(ccw_c_lower *ctx, ccw_node *block, TSNode compound)
{
...
static void lower_statement(ccw_c_lower *ctx, ccw_node *block, TSNode statement)
{
    if (ctx->failed || ctx->rejected || ts_node_is_null(statement)) return;
    if (malformed_node(ctx, statement)) return;
    if (node_is(statement, "comment")) return;
...
static void lower_parameters(ccw_c_lower *ctx, TSNode declarator)
{
    TSNode list = find_descendant(declarator, "parameter_list");
    if (ts_node_is_null(list)) return;
    uint32_t n = ts_node_named_child_count(list);
    for (uint32_t i = 0; i < n && !ctx->failed; i++) {
        TSNode parameter = ts_node_named_child(list, i);
        if (!node_is(parameter, "parameter_declaration")) continue;
        TSNode decl = field(parameter, "declarator");
        TSNode id = declarator_identifier(decl);
        if (ts_node_is_null(id)) continue; /* e.g. `(void)` */
        char *name = node_text(id, ctx->source, ctx->source_len);
        ccw_ir_type type =
            lower_type(field(parameter, "type"), ctx->source, ctx->source_len);
        if (name == NULL ||
            ccw_ir_function_add_param(ctx->ir, ctx->fn, type, name) != CCW_OK)
            lower_fail(ctx, "swaff C: could not lower function parameter");
        free(name);
    }
}

static void lower_function(ccw_c_lower *ctx, TSNode function)
{
                        ...
                        ccw_swaff_report *report, char **error_message)
{
    ...
    if (fe != &g_frontend_c || source == NULL || module_name == NULL) {
        set_error(error_message, "swaff C: invalid frontend, source, or module name");
        return NULL;
    }
    if (source_len > UINT32_MAX) {
        set_error(error_message, "swaff C: source is too large for Tree-sitter");
        return NULL;
    }
    ...
    if (parser == NULL || language == NULL ||
        ...
        set_error(error_message, "swaff C: vendored C grammar is ABI-incompatible");
        return NULL;
    }
    TSTree *tree =
    ...
    for (uint32_t i = 0; i < n && !ctx.failed && !ctx.rejected; i++) {
        ...
        if (node_is(child, "comment") || malformed_node(&ctx, child)) continue;
    ...
    if (ctx.rejected || ctx.failed) {
        const char *message = ctx.failed ? ctx.failure
            : "swaff C: rejected malformed subtree";
