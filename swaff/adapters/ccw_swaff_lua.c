...
static void break_stack_push(ccw_lua_break_stack *bs, const char *merge_name)
{
    if (bs->depth < CCW_LUA_MAX_BREAK)
        bs->merge_blocks[bs->depth++] = lua_strdup(merge_name);
}

static void break_stack_pop(ccw_lua_break_stack *bs)
{
    if (bs->depth > 0) {
...
static const char *break_stack_top(const ccw_lua_break_stack *bs)
{
    return bs->depth > 0 ? bs->merge_blocks[bs->depth - 1] : NULL;
}

static void break_stack_clear(ccw_lua_break_stack *bs)
{
    while (bs->depth > 0) break_stack_pop(bs);
}
...
static char *lower_number(ccw_lua_lower *ctx, ccw_node block, TSNode node)
{
    ...
    temp = new_temp(ctx);
    if (temp == NULL) { free(text); return NULL; }
    if (is_float) {
...
static char *lower_string(ccw_lua_lower *ctx, ccw_node block, TSNode node)
{
    ...
    if (temp == NULL) { free(raw); return NULL; }
...
static char *lower_table(ccw_lua_lower *ctx, ccw_node block, TSNode node)
{
    ...
    for (uint32_t i = 0; i < count && !ctx->failed; i++) {
        ...
        if (node_is(field_node, "table_colon") || node_is(field_node, "table_dot")) {
            ...
            if (fc >= 2) val_node = ts_node_named_child(field_node, fc - 1);
            char *key = node_is(field_node, "table_colon")
                ? lower_expression(ctx, block, key_node)
                : lower_expression(ctx, block, key_node);
            char *val = ts_node_is_null(val_node)
                ? NULL : lower_expression(ctx, block, val_node);
            if (key != NULL && val != NULL) {
                ccw_node set = ccw_ir_instr_build(ctx->ir, "table.set", CCW_TY_VOID);
            ...
            if (val != NULL) {
                ...
                if (idx != NULL &&
                    ccw_kliche_int_const(ctx->ir, block, idx, (int64_t)i) != 0) {
                    ccw_node set = ccw_ir_instr_build(ctx->ir, "table.set", CCW_TY_VOID);
                    ccw_node t = ccw_ir_operand_reg(ctx->ir, temp);
...
static char *lower_field_access(ccw_lua_lower *ctx, ccw_node block, TSNode node)
{
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        if (ts_node_is_null(obj_node)) obj_node = child;
        else if (ts_node_is_null(field_node)) field_node = child;
    }
...
static char *lower_binary(ccw_lua_lower *ctx, ccw_node block, TSNode expr)
{
    ...
    ccw_ir_type result_type =
        ...
        strncmp(opcode, "logic.", 6) == 0 ? CCW_TY_I1 : CCW_TY_I64;
    if (ccw_kliche_binary(ctx->ir, block, opcode, dest, lhs, rhs,
                          result_type) == 0) {
        free(lhs); free(rhs); free(dest); free(op_text);
...
static char *lower_unary(ccw_lua_lower *ctx, ccw_node block, TSNode expr)
{
    ...
    else if (strcmp(op, "not ") == 0 || strcmp(op, "not") == 0) opcode = "logic.not";
    else if (strcmp(op, "#") == 0) opcode = "len";
    ...
    dest = new_temp(ctx);
    if (operand == NULL || dest == NULL) { free(operand); free(dest); return NULL; }
    ccw_ir_type ty = (strcmp(opcode, "logic.not") == 0) ? CCW_TY_I1 : CCW_TY_I64;
...
static char *lower_call(ccw_lua_lower *ctx, ccw_node block, TSNode node)
{
    ...
    if (strcmp(type, "self_call_colon") == 0) {
        ...
        if (ts_node_is_null(prefix)) prefix = first_named_child(node);
        if (ts_node_is_null(args_node)) {
            uint32_t nc = ts_node_named_child_count(node);
            if (nc >= 2) args_node = ts_node_named_child(node, nc - 1);
        }
    }
    ...
    if (is_method) {
        ...
        if (!ts_node_is_null(prefix)) {
            ...
            else if (node_is(child, "table_dot") || node_is(child, "table_colon"))
                callee = lower_field_access(ctx, block, child);
        }
        if (callee == NULL) {
            lower_fail(ctx, "swaff Lua: only direct identifier calls and field accesses are supported");
            return NULL;
        }
    }
    ...
    if (dest != NULL) {
        ...
        if (ins == 0 || ccw_ir_instr_set_dest(ctx->ir, ins, dest) != CCW_OK) {
            ...
            ccw_node target = is_method
                ? ccw_ir_operand_reg(ctx->ir, callee)
                : ccw_ir_operand_func(ctx->ir, callee);
            if (target == 0 || ccw_ir_instr_add_operand(ctx->ir, ins, target) != CCW_OK) {
                free(dest); dest = NULL;
            } else {
                for (int i = 0; i < arg_count; i++) {
                    ccw_node arg = ccw_ir_operand_reg(ctx->ir, args[i]);
                    if (arg == 0 || ccw_ir_instr_add_operand(ctx->ir, ins, arg) != CCW_OK) {
                        free(dest); dest = NULL; break;
                    }
                }
                if (dest != NULL && ccw_ir_block_append_instr(ctx->ir, block, ins) != CCW_OK) {
                    free(dest); dest = NULL;
                }
            }
        }
    ...
    if (!is_method) free(callee);
    free(receiver_reg);
    return dest;
}

static char *lower_ellipsis(ccw_lua_lower *ctx, ccw_node block)
{
    char *temp = new_temp(ctx);
    if (temp == NULL) return NULL;
...
static char *lower_expression(ccw_lua_lower *ctx, ccw_node block, TSNode expr)
{
    ...
    if (strcmp(type, "self_call_colon") == 0) return lower_call(ctx, block, expr);
    if (strcmp(type, "table_dot") == 0 || strcmp(type, "table_colon") == 0)
        return lower_field_access(ctx, block, expr);
    if (strcmp(type, "tableconstructor") == 0) return lower_table(ctx, block, expr);
    if (strcmp(type, "ellipsis") == 0) return lower_ellipsis(ctx, block);
    if (strcmp(type, "function") == 0) {
        ctx->report->unsupported_nodes++;
        lower_fail(ctx, "swaff Lua: anonymous function expressions are not yet supported");
        return NULL;
    }
...
static void lower_if(ccw_lua_lower *ctx, ccw_node *block, TSNode node)
{
    ...
    if (i < count && node_is(ts_node_named_child(node, i), "if_start")) i++;
    ...
    while (i < count && !ctx->failed) {
        ...
        if (then_block == 0 || next_block == 0 || cond == NULL ||
            ccw_kliche_branch_if(ctx->ir, condition_block, cond,
                                 then_name, next_name) == 0) {
            free(cond);
            free(then_name);
            free(next_name);
            if (!ctx->failed)
    ...
    if (!ctx->failed) *block = merge_block;
    free(merge_name);
}

static void lower_while(ccw_lua_lower *ctx, ccw_node *block, TSNode node)
{
    uint32_t count = ts_node_named_child_count(node);
    ...
    if (cond_name == NULL || body_name == NULL || merge_name == NULL) {
        free(cond_name); free(body_name); free(merge_name);
        return;
    }
    ...
    for (uint32_t i = 0; i < count && !ctx->failed; i++) {
        TSNode child = ts_node_named_child(node, i);
        if (node_is(child, "while_do")) { in_body = true; continue; }
        if (node_is(child, "while_end")) continue;
        if (node_is(child, "while_start")) continue;
        if (!in_body) condition = child;
    }
    ...
    for (uint32_t i = 0; i < count && !ctx->failed; i++) {
        TSNode child = ts_node_named_child(node, i);
        if (node_is(child, "while_do")) { in_body = true; continue; }
        if (node_is(child, "while_end")) break;
        if (node_is(child, "while_start")) continue;
        if (in_body) lower_statement(ctx, &body_block, child);
    }
...
static void lower_repeat(ccw_lua_lower *ctx, ccw_node *block, TSNode node)
{
    uint32_t count = ts_node_named_child_count(node);
    TSNode condition = null_node();
    ...
    if (body_name == NULL || merge_name == NULL) {
        free(body_name); free(merge_name);
        return;
    }
    ...
    for (uint32_t i = 0; i < count && !ctx->failed; i++) {
        TSNode child = ts_node_named_child(node, i);
        if (node_is(child, "repeat_start")) continue;
...
static void lower_for(ccw_lua_lower *ctx, ccw_node *block, TSNode node)
{
    uint32_t count = ts_node_named_child_count(node);
    ...
    char *cond_name = NULL, *body_name = NULL, *merge_name = NULL, *incr_name = NULL;
    ...
    if (cond_name == NULL || body_name == NULL || merge_name == NULL || incr_name == NULL) {
        free(cond_name); free(body_name); free(merge_name); free(incr_name);
        return;
    }
    ...
    if (is_generic) {
        for (uint32_t i = 0; i < count && !ctx->failed; i++) {
            TSNode child = ts_node_named_child(node, i);
            if (node_is(child, "for_start")) continue;
            if (node_is(child, "for_in")) continue;
            if (node_is(child, "for_do")) { in_body = true; continue; }
            if (node_is(child, "for_end")) continue;
            if (!in_body && !node_is(child, "for_in")) {
                if (ts_node_is_null(var_node)) var_node = child;
                else if (ts_node_is_null(exprlist_node)) exprlist_node = child;
            }
            if (in_body) lower_statement(ctx, &body_block, child);
        }
        ccw_kliche_jump(ctx->ir, *block, cond_name);
        if (!ts_node_is_null(exprlist_node)) {
            char *iter_result = lower_expression(ctx, cond_block, exprlist_node);
            if (iter_result != NULL) {
                ccw_kliche_branch_if(ctx->ir, cond_block, iter_result, body_name, merge_name);
            }
            free(iter_result);
        }
    } else {
        for (uint32_t i = 0; i < count && !ctx->failed; i++) {
            ...
            if (!in_body) {
                if (ts_node_is_null(var_node)) var_node = child;
                else if (ts_node_is_null(start_node)) start_node = child;
                else if (ts_node_is_null(end_node)) end_node = child;
                else if (ts_node_is_null(step_node)) step_node = child;
            }
            if (in_body) lower_statement(ctx, &body_block, child);
        }
        ...
        if (!ts_node_is_null(var_node) && !ts_node_is_null(end_node)) {
            ...
            if (var_load != NULL && end_val != NULL) {
                ...
                ccw_kliche_binary(ctx->ir, cond_block, "icmp.le",
                    cmp, var_load, end_val, CCW_TY_I1);
                ccw_kliche_branch_if(ctx->ir, cond_block, cmp, body_name, merge_name);
            }
            free(var_load); free(end_val);
        }
    }
    ...
    if (!is_generic && !ts_node_is_null(var_node)) {
        ...
        char *step_val = !ts_node_is_null(step_node)
            ? lower_expression(ctx, incr_block, step_node) : NULL;
        char *one = new_temp(ctx);
        if (one != NULL && ccw_kliche_int_const(ctx->ir, incr_block, one, 1) != 0) {
...
static void lower_break(ccw_lua_lower *ctx, ccw_node block)
{
...
static void lower_goto(ccw_lua_lower *ctx, ccw_node block, TSNode node)
{
...
static void lower_label(ccw_lua_lower *ctx, ccw_node *block, TSNode node)
{
    TSNode label = first_named_child(node);
    char *label_name = node_text(label, ctx->source, ctx->source_len);
    if (label_name == NULL) {
...
static void lower_statement(ccw_lua_lower *ctx, ccw_node *block, TSNode node)
{
    ...
    else if (strcmp(type, "function_call") == 0 || strcmp(type, "self_call_colon") == 0) {
...
static void lower_function(ccw_lua_lower *ctx, TSNode node)
{
    ...
    if (!ts_node_is_null(params)) {
        ...
        for (uint32_t i = 0; i < pc; i++) {
            ...
            if (node_is(p, "ellipsis")) continue;
            if (node_is(p, "identifier")) {
                char *pname = node_text(p, ctx->source, ctx->source_len);
                if (pname != NULL) {
                    ccw_ir_function_add_param(ctx->ir, ctx->fn, CCW_TY_I64, pname);
                    add_local(ctx, pname);
                    free(pname);
                }
            }
...
static void lower_top_level(ccw_lua_lower *ctx, TSNode root)
{
    ...
    if (ctx->fn == 0) {
        lower_fail(ctx, "swaff Lua: could not create top-level module function");
        return;
    }
    clear_locals(ctx);
    ctx->temp_index = 0;
    ...
    for (uint32_t i = 0; i < count && !ctx->failed && !ctx->rejected; i++) {
        ...
        if (node_is(child, "function_statement")) continue;
