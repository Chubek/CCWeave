...
static const char *const CCW_IR_GRAMMAR
    = " str      : /\"(\\\\\\\\.|[^\"])*\"/ ;                                 "
      ...
      " \n"
      " fnum     : "
      "/-?([0-9]+\\.[0-9]+([eE][-+]?[0-9]+)?|[0-9]+([eE][-+]?[0-9]+)?|inf|nan)"
      "/ ; \n"
      " reg      : '%' <ident> ;                                              "
      " \n"
      " fref     : '@' <ident> ;                                              "
...
static bool
tag_has (const mpc_ast_t *ast, const char *token)
{
...
static mpc_ast_t *
child_nth (mpc_ast_t *ast, const char *token, int nth)
{
  int seen = 0;
...
static mpc_ast_t *
child_first (mpc_ast_t *ast, const char *token)
{
...
static void
build_attrs (ccw_ir *ir, ccw_node owner, mpc_ast_t *ast)
{
  for (int i = 0; i < ast->children_num; i++)
    {
      ...
      if (ks != NULL && vs != NULL)
        ccw_ir_attr_set (ir, owner, ks, vs);
      free (ks);
      free (vs);
    }
}
...
static ccw_node
build_operand (ccw_ir *ir, mpc_ast_t *o)
{
  if (tag_has (o, "iconst"))
    {
      ...
      return ccw_ir_operand_const_int (
          ir, t, v ? (int64_t)strtoll (v->contents, NULL, 10) : 0);
    }
  if (tag_has (o, "fconst"))
    {
      mpc_ast_t *ty = child_first (o, "ident");
  ...
  if (tag_has (o, "fref"))
    return ccw_ir_operand_func (ir, sym_name (o));
  if (tag_has (o, "bref"))
    return ccw_ir_operand_block (ir, sym_name (o));
  return ccw_ir_operand_reg (ir, sym_name (o));
}

static bool
build_instr (ccw_ir *ir, ccw_node blk, mpc_ast_t *in)
{
  ...
  for (int i = 0; i < in->children_num; i++)
    {
      ...
      if (tag_has (c, "dest") || tag_has (c, "attr"))
        continue;
      if (tag_has (c, "operand") || tag_has (c, "iconst")
          || tag_has (c, "fconst") || tag_has (c, "reg") || tag_has (c, "fref")
          || tag_has (c, "bref"))
        {
          ccw_node o = build_operand (ir, c);
          if (o != 0)
            ccw_ir_instr_add_operand (ir, ins, o);
        }
    }
...
static bool
build_block (ccw_ir *ir, ccw_node fn, mpc_ast_t *b)
{
  ...
  for (int i = 0; i < b->children_num; i++)
    if (tag_has (b->children[i], "instr"))
      if (!build_instr (ir, blk, b->children[i]))
        return false;
  return true;
}

static bool
build_function (ccw_ir *ir, mpc_ast_t *f)
{
  mpc_ast_t *nm = child_first (f, "fref");
  mpc_ast_t *ty = child_first (f, "ident");
  ccw_ir_type rt = CCW_TY_VOID;
  if (ty != NULL)
    ccw_ir_type_parse (ty->contents, &rt);
      ...
      for (int i = 0; i < ps->children_num; i++)
        {
          ...
          ccw_ir_type t = CCW_TY_VOID;
          if (pt != NULL)
            ccw_ir_type_parse (pt->contents, &t);
  ...
  for (int i = 0; i < f->children_num; i++)
    if (tag_has (f->children[i], "block"))
      if (!build_block (ir, fn, f->children[i]))
        return false;
  return true;
}

static ccw_ir *
build_module (mpc_ast_t *ast, char **error_message)
{
  ...
  if (pfid == NULL || !ccw_profile_parse (pfid->contents, &profile))
    {
      if (error_message)
        *error_message = ccw_ir_strdup (
            "module header must declare (profile tilly|on1x)");
      return NULL;
    }
  ...
  if (ir == NULL)
    {
      if (error_message)
        *error_message = ccw_ir_strdup ("out of memory");
      return NULL;
    }
  build_attrs (ir, 0, root);
  for (int i = 0; i < root->children_num; i++)
    {
      if (!tag_has (root->children[i], "function"))
        continue;
      if (!build_function (ir, root->children[i]))
        {
          ccw_ir_module_destroy (ir);
          if (error_message)
            *error_message = ccw_ir_strdup ("malformed function");
          return NULL;
        }
    }
  return ir;
...
ccw_ir *
ccw_ir_parse (const char *text, char **error_message)
{
  if (error_message)
    *error_message = NULL;
  if (text == NULL)
  ...
  mpc_parser_t *reg = mpc_new ("reg"), *fref = mpc_new ("fref"),
               *bref = mpc_new ("bref");
  ...
  mpc_err_t *gerr
      = mpca_lang (MPCA_LANG_DEFAULT, CCW_IR_GRAMMAR, str_p, ident, inum, fnum,
                   reg, fref, bref, iconst, fconst, operand, attr, dest, instr,
                   block, param, params, function, profile, module, NULL);
  if (gerr != NULL)
    {
      if (error_message)
        {
  ...
  mpc_cleanup (19, str_p, ident, inum, fnum, reg, fref, bref, iconst, fconst,
               operand, attr, dest, instr, block, param, params, function,
               profile, module);
  return ir;
}

ccw_ir *
ccw_ir_parse_file (const char *path, char **error_message)
{
  if (error_message)
    *error_message = NULL;
  ...
  if (fp == NULL)
    {
      if (error_message)
        *error_message = ccw_ir_strdup ("cannot open file");
      return NULL;
    }
