...
#define CCW_OP_IO_OPEN "io.open"
  ...
  ccw_status ccw_ir_attr_set (ccw_ir *ir, ccw_node owner, const char *key,
                              const char *value);
  ...
  const char *ccw_ir_attr_lookup (const ccw_ir *ir, ccw_node owner,
                                  const char *key);

  /* ---------- construction ---------- */

  ccw_node ccw_ir_function_add (ccw_ir *ir, const char *name,
                                ccw_ir_type result);
  ...
  ccw_node ccw_ir_instr_build (ccw_ir *ir, const char *opcode,
                               ccw_ir_type type);
  ccw_status ccw_ir_instr_set_dest (ccw_ir *ir, ccw_node ins,
                                    const char *dest);
  ccw_status ccw_ir_instr_add_operand (ccw_ir *ir, ccw_node ins,
                                       ccw_node operand);
  ccw_status ccw_ir_instr_set_operand (ccw_ir *ir, ccw_node ins, int index,
                                       ccw_node operand);
  ccw_status ccw_ir_block_append_instr (ccw_ir *ir, ccw_node blk,
                                        ccw_node ins);
  ...
  ccw_node ccw_ir_operand_const_int (ccw_ir *ir, ccw_ir_type type,
                                     int64_t value);
  ...
  const char *ccw_ir_function_param_name (const ccw_ir *ir, ccw_node fn,
                                          int idx);
  ccw_ir_type ccw_ir_function_param_type (const ccw_ir *ir, ccw_node fn,
                                          int idx);
  ...
  ccw_node ccw_ir_block_successor_ref (const ccw_ir *ir, ccw_node blk,
                                       int idx);
  int ccw_ir_block_predecessor_count (const ccw_ir *ir, ccw_node blk);
  ccw_node ccw_ir_block_predecessor_ref (const ccw_ir *ir, ccw_node blk,
                                         int idx);
  ...
  const char *ccw_ir_instr_dest (const ccw_ir *ir,
                                 ccw_node ins); /* NULL if none */
  ...
  ccw_status ccw_ir_const_int_value (const ccw_ir *ir, ccw_node n,
                                     int64_t *out);
  ccw_status ccw_ir_const_float_value (const ccw_ir *ir, ccw_node n,
                                       double *out);
  ...
  ccw_status ccw_ir_instr_replace (ccw_ir *ir, ccw_node old_ins,
                                   ccw_node new_ins);
  ccw_status ccw_ir_instr_insert_before (ccw_ir *ir, ccw_node anchor,
                                         ccw_node new_ins);
