...
static void
sb_append (ccw_sb *sb, const char *s)
{
  if (sb->failed || s == NULL)
    return;
  if (kputs (s, &sb->text) == EOF)
    sb->failed = true;
}

static void
sb_printf (ccw_sb *sb, const char *fmt, ...)
{
  va_list ap;
  va_start (ap, fmt);
...
static void
sb_string (ccw_sb *sb, const char *s)
{
  sb_append (sb, "\"");
...
static void
print_attrs (ccw_sb *sb, const ccw_attrs *a, int depth)
{
...
static void
print_operand (ccw_sb *sb, const ccw_ir *ir, ccw_node id)
{
...
static void
print_instr (ccw_sb *sb, const ccw_ir *ir, ccw_node id, int depth)
{
