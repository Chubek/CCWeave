...
static void terminate(ccw_ir *m, ccw_node blk, ccw_node fn)
{
...
int main(void)
{
    ...
    terminate(tilly, tblk, ccw_ir_function_ref(tilly, 0));
    char *err = NULL;
    ...
    ccw_node t2fn = ccw_ir_function_add(tilly2, "f", CCW_TY_I64);
    ccw_node t2blk = ccw_ir_block_add(tilly2, t2fn, "entry");
    ...
    ccw_node ofn = ccw_ir_function_add(on1x, "f", CCW_TY_I64);
    ccw_node oblk = ccw_ir_block_add(on1x, ofn, "entry");
    ccw_tilly_build_reloc(on1x, oblk, "r", "sym", 0);
    terminate(on1x, oblk, ofn);
    err = NULL;
    ...
    ccw_node ofn2 = ccw_ir_function_add(on1x2, "f", CCW_TY_I64);
    CCW_CHECK(ccw_tilly_set_link_section(on1x2, ofn2, ".text") != CCW_OK,
    ...
    ccw_node o3fn = ccw_ir_function_add(o3, "f", CCW_TY_I64);
    ccw_node o3blk = ccw_ir_block_add(o3, o3fn, "entry");
    ccw_on1x_build_safepoint(o3, o3blk);
    terminate(o3, o3blk, o3fn);
