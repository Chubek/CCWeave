...
static bool
reserve_items (void **items, int *capacity, int needed, size_t item_size)
{
  if (needed <= *capacity)
    return true;
  ...
  while (next < needed)
    next *= 2;
  kvec_t (unsigned char) bytes = { 0, (size_t)*capacity * item_size, *items };
...
static s7_pointer
ccw_val_to_s7 (s7_scheme *sc, ccw_val *v)
{
  s7_pointer out;
  switch (v->type)
    {
    case CCW_T_BOOL:
      out = s7_make_boolean (sc, v->as.b);
      break;
    case CCW_T_INT:
      out = s7_make_integer (sc, v->as.i);
      break;
    case CCW_T_FLOAT:
      out = s7_make_real (sc, v->as.f);
      break;
    case CCW_T_STRING:
      out = s7_make_string (sc, v->as.s ? v->as.s : "");
      break;
    case CCW_T_SYMBOL:
      out = s7_make_symbol (sc, v->as.s ? v->as.s : "|nil|");
      break;
    /* Node ids are plain integers in Scheme: kernels only ever hold ints. */
    case CCW_T_NODE:
      out = s7_make_integer (sc, (s7_int)v->as.node);
      break;
    case CCW_T_NIL:
    default:
      out = s7_nil (sc);
...
static bool
s7_to_ccw_val (s7_scheme *sc, s7_pointer p, ccw_val *out)
{
  if (s7_is_null (sc, p))
    {
      *out = ccw_nil ();
  ...
  if (s7_is_boolean (p))
    {
      *out = ccw_bool (s7_boolean (sc, p));
      return true;
    }
  if (s7_is_integer (p))
    {
      *out = ccw_int ((int64_t)s7_integer (p));
      return true;
    }
  if (s7_is_real (p))
    {
      *out = ccw_float (s7_real (p));
      return true;
    }
  if (s7_is_string (p))
    {
      *out = ccw_string (s7_string (p));
      return true;
    }
  if (s7_is_symbol (p))
    {
      *out = ccw_symbol (s7_symbol_name (p));
      return true;
    }
  return false;
}
...
static s7_pointer
ccw_accessor_trampoline (s7_scheme *sc, s7_pointer args)
{
  ...
  if (nargs < acc->min_arity
      || (acc->max_arity >= 0 && nargs > acc->max_arity))
    {
      ...
      return s7_error (
          sc, s7_make_symbol (sc, "wrong-number-of-args"),
          s7_list (sc, 2, s7_make_string (sc, "~A: wrong number of arguments"),
  ...
  if (acc->ex->current_ir == NULL)
    return s7_error (
        sc, s7_make_symbol (sc, CCW_GLUE_TAG),
        s7_list (
            sc, 2,
            s7_make_string (
                sc,
                "~A: glue accessors are only callable during kernel-apply"),
            s7_make_string (sc, acc->scheme_name)));
  ...
  for (s7_pointer p = args; !s7_is_null (sc, p); p = s7_cdr (p), i++)
    {
      if (!s7_to_ccw_val (sc, s7_car (p), &vals.a[i]))
        {
          for (int j = 0; j < i; j++)
            ccw_val_clear (&vals.a[j]);
          kv_destroy (vals);
          return s7_error (
              sc, s7_make_symbol (sc, CCW_GLUE_TAG),
              s7_list (sc, 2,
                       s7_make_string (sc, "~A: value of unsupported type "
                                           "crossed the glue boundary"),
                       s7_make_string (sc, acc->scheme_name)));
        }
    }
...
ccw_status
ccw_glue_register (ccw_executor *ex, const char *scheme_name, int min_arity,
                   int max_arity, ccw_accessor_fn fn, void *host_ctx)
{
  if (ex == NULL || scheme_name == NULL || fn == NULL)
    return CCW_ERR_TYPE;
  ...
  s7_pointer form
      = s7_list (ex->sc, 3, s7_make_symbol (ex->sc, "define"),
                 s7_make_symbol (ex->sc, scheme_name),
                 s7_list (ex->sc, 3, s7_make_symbol (ex->sc, "lambda"),
                          s7_make_symbol (ex->sc, "args"),
                          s7_list (ex->sc, 4, s7_make_symbol (ex->sc, "apply"),
                                   s7_make_symbol (ex->sc, "ccw-glue-invoke"),
                                   ptr, s7_make_symbol (ex->sc, "args"))));
...
ccw_executor_create (void)
{
  ...
  s7_define_function (ex->sc, "ccw-glue-invoke", ccw_accessor_trampoline, 1, 0,
                      true, "internal glue trampoline");
...
void
ccw_executor_destroy (ccw_executor *ex)
{
  if (ex == NULL)
    return;
  for (int i = 0; i < ex->accessor_count; i++)
...
int
ccw_executor_abi_version (const ccw_executor *ex)
{
  return ex ? ex->abi_version : CCW_ERR_ABI;
}

const char *
ccw_executor_name (const ccw_executor *ex)
{
  return ex ? ex->name : "";
}
...
static ccw_kernel *
kernel_get (ccw_executor *ex, int kernel_id)
{
  if (ex == NULL || kernel_id < 0 || kernel_id >= ex->kernel_count)
    return NULL;
...
static s7_pointer
kernel_export (ccw_executor *ex, ccw_kernel *k, const char *name)
{
  s7_pointer call
      = s7_list (ex->sc, 3, s7_make_symbol (ex->sc, "ccw-library-ref"),
                 s7_list (ex->sc, 2, s7_make_symbol (ex->sc, "quote"),
...
set_error (char **error_message, const char *msg)
{
  if (error_message != NULL)
    *error_message = ccw_dup (msg);
}

/* Calls a guard helper and splits its (ok . payload) result. */
static bool
guarded_result (s7_scheme *sc, s7_pointer pair, s7_pointer *payload)
{
  if (!s7_is_pair (pair))
    {
      *payload = s7_nil (sc);
...
static char *
condition_text (s7_scheme *sc, s7_pointer p)
{
  if (s7_is_string (p))
    return ccw_dup (s7_string (p));
...
int
ccw_kernel_load (ccw_executor *ex, const char *path, char **error_message)
{
  if (error_message)
    *error_message = NULL;
  if (ex == NULL || path == NULL)
    return CCW_ERR_LOAD;

  s7_pointer loader = s7_name_to_value (ex->sc, "ccw-load-guarded");
  s7_pointer res = s7_call (
      ex->sc, loader, s7_list (ex->sc, 1, s7_make_string (ex->sc, path)));
  s7_pointer payload = s7_nil (ex->sc);
  if (!guarded_result (ex->sc, res, &payload))
    {
      if (error_message)
        *error_message = condition_text (ex->sc, payload);
      return CCW_ERR_LOAD;
    }
  ...
  static const char *const required[]
      = { "kernel-info", "kernel-capabilities", "kernel-apply" };
  for (size_t i = 0; i < sizeof (required) / sizeof (required[0]); i++)
    {
      ...
      if (!s7_is_procedure (p))
        {
          char msg[160];
          snprintf (msg, sizeof (msg), "kernel does not export %s",
                    required[i]);
...
ccw_status
ccw_kernel_unload (ccw_executor *ex, int kernel_id)
{
  ...
  free (k->library_key);
  k->library_key = NULL;
  for (int i = 0; i < k->capability_count; i++)
    free (k->capabilities[i]);
  free (k->capabilities);
  k->capabilities = NULL;
...
static char *
info_field (ccw_executor *ex, s7_pointer alist, const char *key)
{
  for (s7_pointer p = alist; s7_is_pair (p); p = s7_cdr (p))
    {
      ...
      if (!s7_is_symbol (k) || strcmp (s7_symbol_name (k), key) != 0)
        continue;
      s7_pointer v = s7_cdr (entry);
      if (s7_is_string (v))
        return ccw_dup (s7_string (v));
      if (s7_is_symbol (v))
        return ccw_dup (s7_symbol_name (v));
...
ccw_status
ccw_kernel_info (ccw_executor *ex, int kernel_id, char **name, char **version,
                 char **description)
{
  if (name)
    *name = NULL;
  ...
  if (!guarded_result (
          ex->sc, s7_call (ex->sc, guard, s7_list (ex->sc, 1, fn)), &alist))
    return CCW_ERR_KERNEL;
  if (!s7_is_pair (alist))
    return CCW_ERR_KERNEL;

  if (name)
    *name = info_field (ex, alist, "name");
  if (version)
    *version = info_field (ex, alist, "version");
  if (description)
    *description = info_field (ex, alist, "description");
  return CCW_OK;
}
...
static int
refresh_capabilities (ccw_executor *ex, ccw_kernel *k)
{
  for (int i = 0; i < k->capability_count; i++)
    free (k->capabilities[i]);
  free (k->capabilities);
  ...
  if (!guarded_result (
          ex->sc, s7_call (ex->sc, guard, s7_list (ex->sc, 1, fn)), &caps))
    return CCW_ERR_KERNEL;
  ...
  int cap = 0;
  if (!reserve_items ((void **)&k->capabilities, &cap, n,
...
int
ccw_kernel_capability_count (ccw_executor *ex, int kernel_id)
{
  ccw_kernel *k = kernel_get (ex, kernel_id);
  if (k == NULL)
    return CCW_ERR_LOAD;
...
const char *
ccw_kernel_capability (ccw_executor *ex, int kernel_id, int idx)
{
  ccw_kernel *k = kernel_get (ex, kernel_id);
  if (k == NULL)
    return NULL;
  if (k->capabilities == NULL && refresh_capabilities (ex, k) < 0)
    return NULL;
  if (idx < 0 || idx >= k->capability_count)
    return NULL;
...
static s7_pointer
options_alist (s7_scheme *sc, const char *const *options)
{
...
ccw_status
ccw_kernel_apply (ccw_executor *ex, int kernel_id, const char *capability,
                  ccw_ir *ir, const char *const *options, char **error_message)
{
  if (error_message)
    *error_message = NULL;
  ccw_kernel *k = kernel_get (ex, kernel_id);
  if (k == NULL || capability == NULL)
    return CCW_ERR_LOAD;
  ...
  bool found = false;
  for (int i = 0; i < n; i++)
    if (strcmp (k->capabilities[i], capability) == 0)
      {
        found = true;
        break;
      }
  if (!found)
    {
      set_error (error_message,
                 "kernel does not provide the requested capability");
  ...
  s7_pointer args = s7_list (ex->sc, 3, s7_make_symbol (ex->sc, capability),
