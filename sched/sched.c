...
static void
fail (ccw_sched_error *e, int code, const char *message)
{
  if (e)
    {
      e->code = code;
...
xstrdup (const char *s)
{
  ...
  if (!s || kputs (s, &copy) == EOF)
    return NULL;
  return ks_release (&copy);
}
static int
reserve (void **p, size_t *capacity, size_t need, size_t item_size)
{
  size_t n;
  kvec_t (unsigned char) bytes;
  if (need <= *capacity && *p != NULL)
    return 1;
  ...
  if (kv_resize (unsigned char, bytes, n *item_size) == NULL)
    return 0;
  *p = bytes.a;
  *capacity = n;
  return 1;
...
static int
clean_scalar (const char *s)
{
  return s && *s && !strchr (s, '\n') && !strchr (s, '\r') && !strchr (s, ',');
}
static int
add_cap (kernel *k, const char *cap)
{
  if (!reserve ((void **)&k->caps, &k->cc, k->nc + 1, sizeof (*k->caps)))
    return 0;
  k->caps[k->nc++] = xstrdup (cap);
  return k->caps[k->nc - 1] != NULL;
}
static int
append_kernel_match (kernel_matches *matches, size_t kernel_index)
{
  size_t i;
  for (i = 0; i < matches->n; ++i)
    if (matches->items[i] == kernel_index)
      return 1;
  if (!reserve ((void **)&matches->items, &matches->c, matches->n + 1,
...
static int
index_kernel (ccw_sched *s, size_t kernel_index)
{
  ...
  if (ret < 0)
    return 0;
  if (ret != 0)
    kh_value (s->kernel_by_name, slot) = kernel_index;
  for (size_t i = 0; i < k->nc; ++i)
    {
      slot = kh_get (sched_capability_index, s->capability_index, k->caps[i]);
      if (slot == kh_end (s->capability_index))
        {
          slot = kh_put (sched_capability_index, s->capability_index,
                         k->caps[i], &ret);
          if (ret < 0)
            return 0;
          if (ret != 0)
            kh_value (s->capability_index, slot) = (kernel_matches){ 0 };
        }
      if (!append_kernel_match (&kh_value (s->capability_index, slot),
                                kernel_index))
        return 0;
    }
  return 1;
...
static int
index_ruleset (ccw_sched *s, size_t ruleset_index)
{
  khint_t slot;
  int ret;
  ...
  if (ret < 0)
    return 0;
  if (ret != 0)
    kh_value (s->ruleset_by_name, slot) = ruleset_index;
  return 1;
}
static int
finish_kernel (ccw_sched *s, kernel *pending)
{
  size_t index;
  if (!pending->name)
    return 1;
  if (!pending->path || !clean_scalar (pending->name))
    return 0;
  if (!reserve ((void **)&s->kernels, &s->ck, s->nk + 1, sizeof (*s->kernels)))
    return 0;
  index = s->nk;
...
static int
load_kernels (ccw_sched *s, ccw_sched_error *e)
{
  char file[1024], line[2048];
  FILE *f;
  kernel current = { 0 };
  int in_caps = 0;
  ...
  if (!f)
    {
      fail (e, 2, "cannot read Kernel.yaml");
      return 0;
    }
  while (fgets (line, sizeof (line), f))
    {
      ...
      if (!strncmp (p, "- path:", 7))
        {
          if (!finish_kernel (s, &current))
            goto bad;
          current.path = xstrdup (strip (p + 7));
          in_caps = 0;
        }
      else if (current.path && !strncmp (p, "name:", 5))
        current.name = xstrdup (strip (p + 5));
      else if (current.path && !strcmp (p, "capabilities:"))
        in_caps = 1;
      else if (in_caps && !strncmp (p, "- ", 2))
        {
          if (!add_cap (&current, strip (p + 2)))
            goto bad;
        }
      else if (*p && p[0] != '#')
        in_caps = 0;
  ...
  if (!s->nk)
    {
      fail (e, 2, "Kernel.yaml has no kernels");
      return 0;
    }
  return 1;
bad:
  ...
  free (current.name);
  free (current.path);
  for (size_t i = 0; i < current.nc; ++i)
    free (current.caps[i]);
  free (current.caps);
  fail (e, 2, "malformed Kernel.yaml");
  return 0;
}
static int
load_rules (ccw_sched *s, ccw_sched_error *e)
{
  char file[1024], line[2048];
  FILE *f;
  ...
  if (!f)
    {
      fail (e, 2, "cannot read Stdrewrite.yaml");
      return 0;
    }
  while (fgets (line, sizeof (line), f))
    {
      ...
      if (!strncmp (p, "- path:", 7))
        {
          ...
          free (path);
          name = NULL;
          path = xstrdup (strip (p + 7));
          if (!path || !clean_scalar (path))
            goto bad;
        }
      else if (!strncmp (p, "name:", 5))
        {
          if (!path)
            goto bad;
          name = xstrdup (strip (p + 5));
          if (!name || !clean_scalar (name)
              || !reserve ((void **)&s->rules, &s->cr, s->nr + 1,
                           sizeof (*s->rules)))
            goto bad;
  ...
  if (!s->nr)
    {
      fail (e, 2, "Stdrewrite.yaml has no rulesets");
      return 0;
    }
  return 1;
bad:
  ...
  free (path);
  fclose (f);
  fail (e, 2, "malformed Stdrewrite.yaml");
  return 0;
}
static int
has_cap (const kernel *k, const char *cap)
{
  for (size_t i = 0; i < k->nc; ++i)
    if (!strcmp (k->caps[i], cap))
      return 1;
  return 0;
}
static int
wildcard (const char *pattern, const char *text)
{
  if (!*pattern)
    return !*text;
  if (*pattern == '*')
    return wildcard (pattern + 1, text)
           || (*text && wildcard (pattern, text + 1));
  return *text && *pattern == *text && wildcard (pattern + 1, text + 1);
}
static int
can_mutate (ccw_sched *s, ccw_sched_error *e)
{
  if (!s || s->sealed)
    {
      fail (e, 3, "scheduler is sealed");
      return 0;
    }
  return 1;
}
static int
add_edge_raw (ccw_sched *s, unsigned from, unsigned to)
{
  ...
  if (slot != kh_end (s->edge_set))
    return 1;
  if (!reserve ((void **)&s->edges, &s->ce, s->ne + 1, sizeof (*s->edges)))
    return 0;
  ...
  s->edges[s->ne++] = (edge){ from, to };
  return 1;
}
static int
add_node (ccw_sched *s, unsigned kind, const char *name, char **members,
          size_t nm, uint32_t *out, ccw_sched_error *e)
{
  node *n;
  if (!can_mutate (s, e) || !clean_scalar (name)
      || !reserve ((void **)&s->nodes, &s->cn, s->nn + 1, sizeof (*s->nodes)))
    {
      fail (e, 1, "out of memory or invalid node name");
      return 0;
    }
  n = &s->nodes[s->nn];
  memset (n, 0, sizeof (*n));
  n->id = (unsigned)s->nn + 1;
  n->kind = kind;
  n->name = xstrdup (name);
  n->members = members;
  n->nm = nm;
  ...
  if (kind != NODE_BARRIER)
    for (size_t i = 0; i < s->nn; ++i)
      if (s->nodes[i].kind == NODE_BARRIER
          && !add_edge_raw (s, s->nodes[i].id, n->id))
        {
          fail (e, 1, "out of memory");
          return 0;
        }
...
static char **
copy_strings (char **items, size_t n)
{
  kvec_t (char *) values = { 0, 0, NULL };
  if (kv_resize (char *, values, n ? n : 1) == NULL)
    return NULL;
  char **copy = values.a;
...
ccw_sched *
ccw_sched_new (const char *name, const char *manifest_dir, ccw_sched_error *e)
{
  ccw_sched *s;
  if (!clean_scalar (name))
    {
      fail (e, 1, "invalid pipeline name");
      return NULL;
    }
  s = calloc (1, sizeof (*s));
  if (!s)
  ...
  s->name = xstrdup (name);
  s->manifest_dir = xstrdup (manifest_dir ? manifest_dir : "manifests");
...
void
ccw_sched_free (ccw_sched *s)
{
  if (!s)
    return;
  for (size_t i = 0; i < s->nk; i++)
    {
      free (s->kernels[i].name);
      free (s->kernels[i].path);
      for (size_t j = 0; j < s->kernels[i].nc; j++)
        free (s->kernels[i].caps[j]);
      free (s->kernels[i].caps);
    }
  ...
  for (size_t i = 0; i < s->nn; i++)
    {
      free (s->nodes[i].name);
      for (size_t j = 0; j < s->nodes[i].nm; j++)
        free (s->nodes[i].members[j]);
      free (s->nodes[i].members);
    }
  if (s->capability_index)
    {
      for (khint_t i = kh_begin (s->capability_index);
           i != kh_end (s->capability_index); ++i)
        if (kh_exist (s->capability_index, i))
          free (kh_value (s->capability_index, i).items);
    }
...
int
ccw_sched_require_kernel (ccw_sched *s, const char *name, uint32_t *out,
                          ccw_sched_error *e)
{
  khint_t slot;
  size_t index;
  if (!s || !name || !out || !s->kernel_by_name)
    {
      fail (e, 1, "invalid require");
      return 0;
    }
  ...
  if (slot != kh_end (s->kernel_by_name))
    {
      ...
      char **caps
          = copy_strings (s->kernels[index].caps, s->kernels[index].nc);
      if (!caps)
        {
          fail (e, 1, "out of memory");
          return 0;
        }
      return add_node (s, NODE_KERNEL, s->kernels[index].name, caps,
                       s->kernels[index].nc, out, e);
    }
  fail (e, 4, "kernel is absent from Kernel.yaml");
  return 0;
}
int
ccw_sched_require_capability (ccw_sched *s, const char *cap,
                              const char *prefer, uint32_t *out,
                              ccw_sched_error *e)
{
  ...
  size_t found, i;
  if (!s || !cap || !out || !s->capability_index)
    {
      fail (e, 1, "invalid capability query");
      return 0;
    }
  slot = kh_get (sched_capability_index, s->capability_index, cap);
  if (slot == kh_end (s->capability_index))
    {
      fail (e, 4, "capability is absent from Kernel.yaml");
      return 0;
    }
  ...
  for (i = 0; i < found; ++i)
    {
      ...
      if (!match || (prefer && !strcmp (prefer, candidate->name)))
        match = candidate;
    }
  if (!found)
    {
  ...
  if (prefer && (!match || strcmp (match->name, prefer)))
    {
      fail (e, 4, "preferred kernel does not provide capability");
      return 0;
    }
  if (!prefer && found != 1)
    {
      fail (e, 4, "capability is ambiguous; supply prefer");
      return 0;
    }
  {
...
int
ccw_sched_probe_capability (ccw_sched *s, const char *cap, const char *prefer,
                            uint32_t *out)
{
  ccw_sched_error e;
  return ccw_sched_require_capability (s, cap, prefer, out, &e);
}
int
ccw_sched_rewrite (ccw_sched *s, const char *pattern, uint32_t *out,
                   ccw_sched_error *e)
{
  char **members = NULL;
  size_t n = 0, cap = 0;
  if (!s || !pattern || !out || !can_mutate (s, e))
    {
      free (members);
      return 0;
    }
  for (size_t i = 0; i < s->nr; i++)
    if (wildcard (pattern, s->rules[i].name))
      {
        if (!reserve ((void **)&members, &cap, n + 1, sizeof (*members)))
          {
            fail (e, 1, "out of memory");
            goto no;
          }
        members[n++] = xstrdup (s->rules[i].name);
      }
  if (!n)
    {
      fail (e, 4, "rewrite pattern matched no rulesets");
      goto no;
    }
  if (!add_node (s, NODE_REWRITE, pattern, members, n, out, e))
    goto no;
  return 1;
no:
  for (size_t i = 0; i < n; i++)
...
int
ccw_sched_edge (ccw_sched *s, uint32_t a, uint32_t b, ccw_sched_error *e)
{
  if (!can_mutate (s, e) || !a || !b || a > s->nn || b > s->nn)
    {
      fail (e, 5, "edge references a node outside this plan");
      return 0;
    }
  if (!add_edge_raw (s, a, b))
    {
      fail (e, 1, "out of memory");
      return 0;
    }
...
int
ccw_sched_barrier (ccw_sched *s, const char *label, uint32_t *out,
                   ccw_sched_error *e)
{
  unsigned id;
  if (!add_node (s, NODE_BARRIER, label ? label : "barrier", NULL, 0, out, e))
    return 0;
  id = *out;
  for (unsigned i = 1; i < id; i++)
    {
      if (!add_edge_raw (s, i, id))
        {
          fail (e, 1, "out of memory");
          return 0;
        }
...
static int
acyclic (const ccw_sched *s)
{
  unsigned *in = calloc (s->nn, sizeof (*in)), seen = 0;
  if (!in)
    return 0;
  ...
  for (;;)
    {
      ...
      if (!id)
        break;
      ++seen;
      for (size_t j = 0; j < s->ne; j++)
        if (s->edges[j].from == id && in[s->edges[j].to - 1] != UINT_MAX)
          --in[s->edges[j].to - 1];
    }
  free (in);
  return seen == s->nn;
...
int
ccw_sched_seal (ccw_sched *s, ccw_plan **out, ccw_sched_error *e)
{
  kstring_t text = { 0, 0, NULL };
  int useful = 0;
  if (!can_mutate (s, e) || !out)
    return 0;
  for (size_t i = 0; i < s->nn; i++)
    if (s->nodes[i].kind != NODE_BARRIER)
      useful = 1;
  if (!useful || !acyclic (s))
    {
      fail (e, 6,
            useful ? "plan contains a cycle" : "plan contains no work nodes");
      return 0;
    }
  ...
  for (size_t i = 0; i < s->nn; i++)
    {
      ...
      for (size_t j = 0; j < n->nm; j++)
        if (ksprintf (&text, " %s", n->members[j]) < 0)
          goto oom;
      if (kputc ('\n', &text) == EOF)
        goto oom;
    }
...
const char *
ccw_plan_text (const ccw_plan *p)
{
  return p ? p->text : "";
}
ccw_plan *
ccw_plan_from_text (const char *text)
{
  ccw_plan *p = malloc (sizeof (*p));
  if (p)
    p->text = xstrdup (text ? text : "");
  return p;
}
int
ccw_plan_write (const ccw_plan *p, const char *path, ccw_sched_error *e)
{
  FILE *f;
  if (!p || !path)
    {
      fail (e, 1, "invalid plan path");
      return 0;
    }
  f = fopen (path, "wb");
  if (!f)
    {
      fail (e, 7, "cannot write plan");
      return 0;
    }
  if (fputs (p->text, f) < 0 || fclose (f))
    {
      fail (e, 7, "cannot write plan");
      return 0;
    }
...
static void
free_node_array (node *nodes, size_t count)
{
  if (nodes == NULL)
    return;
  for (size_t i = 0; i < count; ++i)
    {
      free (nodes[i].name);
      for (size_t j = 0; j < nodes[i].nm; ++j)
        free (nodes[i].members[j]);
...
static int
parse_plan_text (const char *text, node **nodes_out, size_t *nn_out,
                 edge **edges_out, size_t *ne_out, ccw_sched_error *e)
{
  ...
  if (ne_out)
    *ne_out = 0;
  if (text == NULL || nodes_out == NULL || nn_out == NULL || edges_out == NULL
      || ne_out == NULL)
    {
      ...
      if (!strcmp (item, "name"))
      ...
      else if (!strcmp (item, "node"))
        {
          ...
          if (id_text == NULL || kind_text == NULL || name == NULL
              ...
              || sscanf (kind_text, "%u", &kind) != 1 || id != nn + 1
              || (kind != NODE_KERNEL && kind != NODE_REWRITE
                  && kind != NODE_BARRIER)
              || !clean_scalar (name))
            {
              ...
              if (!clean_scalar (extra)
                               ...
                               sizeof (*members)))
                {
                  ...
                  fail (e, !clean_scalar (extra) ? 8 : 1,
                        !clean_scalar (extra) ? "malformed plan member"
                                              : "out of memory");
                  goto done;
                }
      ...
      else if (!strcmp (item, "edge"))
        {
          ...
          if (from_text == NULL || to_text == NULL
              ...
              || !have_space)
            {
              fail (e, !have_space ? 1 : 8, "malformed plan edge");
              goto done;
            }
...
static char *
path_join3 (const char *a, const char *b, const char *c)
{
  kstring_t out = { 0, 0, NULL };
  if (a == NULL || b == NULL || c == NULL)
    return NULL;
  if (ksprintf (&out, "%s%s%s", a,
                (a[0] && a[strlen (a) - 1] == '/') ? "" : "/", b)
          < 0
      || ksprintf (&out, "%s%s", (out.l && out.s[out.l - 1] == '/') ? "" : "/",
                   c)
             < 0)
    {
      free (out.s);
...
static char *
manifest_parent (const char *manifest_dir)
{
  char *parent, *slash;
  size_t n;
  if (manifest_dir == NULL || !*manifest_dir)
    return NULL;
...
static char *
ruleset_file_path (const char *manifest_dir, const char *ruleset_path)
{
  char *candidate, *parent;
  FILE *probe;
  if (ruleset_path == NULL || !*ruleset_path)
    return NULL;
  if (ruleset_path[0] == '/')
    {
...
static int
find_node_rule (const ccw_sched *s, const char *name, size_t *index)
{
  khint_t slot;
  if (s == NULL || name == NULL || index == NULL)
    return 0;
  slot = kh_get (sched_ruleset_by_name, s->ruleset_by_name, name);
  if (slot == kh_end (s->ruleset_by_name))
    return 0;
...
int
ccw_plan_apply_rewrites (const ccw_plan *plan, ccw_ir *ir,
                         const char *manifest_dir, ccw_oeuph_budget budget,
                         ccw_cost_model model, ccw_oeuph_stats *stats,
                         ...
                         ccw_sched_error *e)
{
  ...
  for (size_t i = 0; i < nn; ++i)
    if (nodes[i].kind == NODE_REWRITE)
      required += nodes[i].nm;
  if (stats_count != NULL)
    *stats_count = required;
          ...
          for (size_t j = 0; j < nodes[selected].nm; ++j)
            {
              ...
              if (!find_node_rule (manifest, nodes[selected].members[j],
                                   &ruleset_index))
                {
                  fail (
                      e, 8,
                      "plan references a ruleset absent from Stdrewrite.yaml");
                  free (done);
                  ccw_sched_free (manifest);
                  free_node_array (nodes, nn);
                  free (edges);
              ...
              if (path == NULL)
                {
                  fail (e, 2,
                        "ruleset path from Stdrewrite.yaml is unreadable");
                  free (done);
                  ccw_sched_free (manifest);
                  free_node_array (nodes, nn);
                  free (edges);
 ...
 */
static unsigned
rotr (unsigned x, unsigned n)
{
  return (x >> n) | (x << (32 - n));
}
static void
sha256_block (unsigned state[8], const unsigned char block[64])
{
...
int
ccw_plan_hash (const ccw_plan *p, char out[65])
{
  unsigned state[8] = { 0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a,
  ...
  full = n / 64;
  for (i = 0; i < full; i++)
    sha256_block (state, (const unsigned char *)p->text + i * 64);
  n -= full * 64;
  memset (block, 0, sizeof (block));
  memcpy (block, p->text + full * 64, n);
  block[n] = 0x80;
  if (n >= 56)
    {
  ...
  sha256_block (state, block);
  for (i = 0; i < 8; i++)
    sprintf (out + i * 8, "%08x", state[i]);
  out[64] = 0;
  return 1;
}
int
ccw_plan_check (const char *path, const char *dir, ccw_sched_error *e)
{
  FILE *f;
  char line[4096];
  ccw_sched *s;
  ...
  if (!f)
    {
      fail (e, 7, "cannot read plan");
      return 0;
    }
  if (!fgets (line, sizeof line, f) || strcmp (line, "CCW-SCHED-PLAN 1\n"))
    {
      fclose (f);
      fail (e, 8, "invalid plan header");
      return 0;
    }
  s = ccw_sched_new ("check", dir, e);
  if (!s)
    {
      fclose (f);
      return 0;
  ...
  while (fgets (line, sizeof line, f))
    {
      char kind[16], name[2048];
      unsigned id, type;
      if (sscanf (line, "node %u %u %2047s", &id, &type, name) == 3)
        {
          char *members = strstr (line, name);
          size_t index;
          if (!members)
            {
              ok = 0;
          ...
          if (type == NODE_KERNEL)
            {
              khint_t slot
                  = kh_get (sched_kernel_by_name, s->kernel_by_name, name);
              if (slot == kh_end (s->kernel_by_name))
                ok = 0;
              else
                index = kh_value (s->kernel_by_name, slot);
              while (ok && *members)
                {
                  char cap[512];
                  if (sscanf (members, " %511s", cap) != 1)
                    break;
                  if (!has_cap (&s->kernels[index], cap))
                    ok = 0;
          ...
          else if (type == NODE_REWRITE)
            {
              while (ok && *members)
                {
                  char rule[512];
                  if (sscanf (members, " %511s", rule) != 1)
                    break;
                  if (kh_get (sched_ruleset_by_name, s->ruleset_by_name, rule)
                      == kh_end (s->ruleset_by_name))
                    ok = 0;
                  members = strchr (members + 1, ' ');
                  if (!members)
                    break;
  ...
  ccw_sched_free (s);
  if (!ok)
    fail (e, 8, "plan is invalidated by manifest drift");
  return ok;
}
