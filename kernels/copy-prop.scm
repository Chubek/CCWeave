...
(define-library ((ccweave kernel copy-prop))
  ...
  (begin
    ...
    (define (kernel-info)
      '((name        . copy-prop)
        (version     . "1.0.0")
        (description . "Replaces uses of copies with their original values.")))
    ...
    (define (kernel-apply capability ir options)
      (unless (eq? capability 'opt.copy-propagation)
        (error "copy-prop: unsupported capability" capability))
