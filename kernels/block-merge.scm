...
(define-library ((ccweave kernel block-merge))
  ...
  (begin
    ...
    (define (kernel-info)
      '((name        . block-merge)
        (version     . "1.0.0")
        (description . "Merges blocks connected by a single unconditional edge.")))
    ...
    (define (kernel-apply capability ir options)
      (unless (eq? capability 'opt.block-merging)
        (error "block-merge: unsupported capability" capability))
