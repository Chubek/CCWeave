;;; inline.scm
;;; CCWeave Kernel: Inlines call sites under a size budget given in options.
...
(define-library ((ccweave kernel inline))
  ...
  (begin
    ...
    (define (kernel-info)
      '((name        . inline)
        (version     . "1.0.0")
        (description . "Inlines call sites under a size budget given in options.")))
    ...
    (define (kernel-apply capability ir options)
      (unless (eq? capability 'opt.inlining)
        (error "inline: unsupported capability" capability))
