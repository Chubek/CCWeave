...
(define-library ((ccweave kernel reaching-defs))
  ...
  (begin
    ...
    (define (kernel-info)
      '((name        . reaching-defs)
        (version     . "1.0.0")
        (description . "Computes reaching-definition sets per block.")))

    (define (kernel-capabilities)
      '(analysis.reaching-definitions))

    (define (kernel-apply capability ir options)
      (unless (eq? capability 'analysis.reaching-definitions)
        (error "reaching-defs: unsupported capability" capability))
      (let ((result '()))
