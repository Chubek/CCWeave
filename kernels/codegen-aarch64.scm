;;; codegen-aarch64.scm
;;; CCWeave Kernel: Emits AArch64 machine code from Tilly-profile modules.
...
(define-library ((ccweave kernel codegen-aarch64))
  ...
  (begin
    ...
    (define (kernel-info)
      '((name        . codegen-aarch64)
        (version     . "1.0.0")
        (description . "Emits AArch64 machine code from Tilly-profile modules.")))

    (define (kernel-capabilities)
      '(codegen.aarch64))

    (define (kernel-apply capability ir options)
      (unless (eq? capability 'codegen.aarch64)
        (error "codegen-aarch64: unsupported capability" capability))
