;;; cps-convert.scm
;;; CCWeave Kernel: Rewrites functions into continuation-passing style.
...
(define-library ((ccweave kernel cps-convert))
  ...
  (begin
    ...
    (define (kernel-info)
      '((name        . cps-convert)
        (version     . "1.0.0")
        (description . "Rewrites functions into continuation-passing style.")))

    (define (kernel-capabilities)
      '(lower.cps-conversion))
    ...
    (define (kernel-apply capability ir options)
      (unless (eq? capability 'lower.cps-conversion)
        (error "cps-convert: unsupported capability" capability))
