;;; load-store-forward.scm
;;; CCWeave Kernel: Forwards stored values to subsequent loads of the same location.
...
(define-library ((ccweave kernel load-store-forward))
  ...
  (begin
    ...
    (define (kernel-info)
      '((name        . load-store-forward)
        (version     . "1.0.0")
        (description . "Forwards stored values to subsequent loads of the same location.")))

    (define (kernel-capabilities)
      '(opt.load-store-forwarding))
    ...
    (define (kernel-apply capability ir options)
      (unless (eq? capability 'opt.load-store-forwarding)
        (error "load-store-forward: unsupported capability" capability))
