;;; dce.scm
;;; CCWeave Kernel: Removes instructions whose results are never used and have no effects.
...
(define-library ((ccweave kernel dce))
  ...
  (begin
    ...
    (define (kernel-info)
      '((name        . dce)
        (version     . "1.0.0")
        (description . "Removes instructions whose results are never used and have no effects.")))
    ...
    (define (kernel-apply capability ir options)
      (unless (eq? capability 'opt.dead-code-elimination)
        (error "dce: unsupported capability" capability))
