  ...
  (begin
    ...
    (define (kernel-info)
      '((name        . bounds-check-elim)
        (version     . "1.0.0")
        (description . "Removes bounds checks proven redundant by value-range facts.")))

    (define (kernel-capabilities)
      '(opt.bounds-check-elimination))
    ...
    (define (kernel-apply capability ir options)
      (unless (eq? capability 'opt.bounds-check-elimination)
        (error "bounds-check-elim: unsupported capability" capability))
