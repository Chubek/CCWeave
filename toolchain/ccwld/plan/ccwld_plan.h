...
extern "C"
{
  ...
  typedef int (*ccwld_hook_fn) (ccwld_phase_id phase, struct ccwld_link *link,
                                void *user);
  ...
  int ccwld_plan_symbol (ccwld_plan *p, const char *name,
                         struct ccwld_expr *expr, int provide, int hidden,
                         ccwld_error *e);
  ...
  struct ccwld_expr *ccwld_expr_align_expr (struct ccwld_expr *a,
                                            uint64_t boundary);
