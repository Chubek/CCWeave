...
static int grow(void **p, size_t *cap, size_t n, size_t sz) {
    if (n <= *cap) return 1;
...
static int valid_kind(const char *k) {
    return k && (!strcmp(k, "exe") || !strcmp(k, "dso") ||
                 !strcmp(k, "reloc") || !strcmp(k, "pie"));
}

static int is_sealed(const ccwld_plan *p, ccwld_error *e) {
    if (!p || p->sealed) {
        if (e) { e->code = 1; snprintf(e->message, sizeof(e->message),
...
void ccwld_plan_free(ccwld_plan *p) {
    ...
    /* inputs */
    for (size_t i = 0; i < p->ninputs; i++) free(p->inputs[i].path);
    free(p->inputs);

    /* paths */
    for (size_t i = 0; i < p->npaths; i++) free(p->paths[i]);
    free(p->paths);
...
int ccwld_plan_output(ccwld_plan *p, const ccwld_output *o, ccwld_error *e) {
    if (is_sealed(p, e)) return 0;
    if (!o || !valid_kind(o->kind) || !valid_format(o->format)) {
        if (e) { e->code = 2; snprintf(e->message, sizeof(e->message),
    ...
    free((void *)p->output.kind);   free((void *)p->output.format);
    free((void *)p->output.entry);  free((void *)p->output.soname);
    free((void *)p->output.osabi);
...
int ccwld_plan_input(ccwld_plan *p, const char *path, int as_needed,
    ...
    if (!grow((void **)&p->inputs, &p->cinputs, p->ninputs + 1,
              sizeof(*p->inputs))) {
        if (e) { e->code = 3; snprintf(e->message, sizeof(e->message),
                                        "out of memory"); }
        return 0;
    }
...
int ccwld_plan_group(ccwld_plan *p, const char **paths, size_t n,
    ...
    for (size_t i = 0; i < n; i++) {
        if (!ccwld_plan_input(p, paths[i], 0, 0, e)) return 0;
...
int ccwld_plan_section(ccwld_plan *p, const char *name, const char *region,
    ...
    return ccwld_plan_section_full(p, name, region, align, selector,
                                   NULL, at_region, NULL, e);
}
...
static void json_escape(char **s, size_t *len, size_t *cap, const char *str) {
    if (!str) { APPEND(*s, *len, *cap, "null"); return; }
    APPEND(*s, *len, *cap, "\"");
    for (const char *p = str; *p; p++) {
        switch (*p) {
        case '"':  APPEND(*s, *len, *cap, "\\\""); break;
        case '\\': APPEND(*s, *len, *cap, "\\\\"); break;
        case '\n': APPEND(*s, *len, *cap, "\\n");  break;
        case '\r': APPEND(*s, *len, *cap, "\\r");  break;
        case '\t': APPEND(*s, *len, *cap, "\\t");  break;
        default: {
            char c[2] = {*p, 0};
...
int ccwld_plan_serialize(const ccwld_plan *p, char **out, size_t *out_len,
                         ccwld_error *e) {
    if (!p || !out) {
        if (e) { e->code = 1; snprintf(e->message, sizeof(e->message),
    ...
    for (size_t i = 0; i < p->ninputs; i++) {
        if (i) APPEND(s, len, cap, ",");
        APPEND(s, len, cap, "{\"path\":");
        json_escape(&s, &len, &cap, p->inputs[i].path);
        APPEND(s, len, cap, ",\"as_needed\":%s", p->inputs[i].as_needed ? "true" : "false");
        APPEND(s, len, cap, ",\"startup\":%s", p->inputs[i].startup ? "true" : "false");
        APPEND(s, len, cap, ",\"is_group\":%s", p->inputs[i].is_group ? "true" : "false");
        APPEND(s, len, cap, "}");
    }
    APPEND(s, len, cap, "]");
    ...
    for (size_t i = 0; i < p->nsyms; i++) {
        ...
        APPEND(s, len, cap, ",\"provide\":%s", p->syms[i].provide ? "true" : "false");
        APPEND(s, len, cap, ",\"hidden\":%s", p->syms[i].hidden ? "true" : "false");
    ...
    for (size_t i = 0; i < p->nvers; i++) {
        ...
        APPEND(s, len, cap, ",\"is_default\":%s", p->vers[i].is_default ? "true" : "false");
        APPEND(s, len, cap, "}");
    }
    APPEND(s, len, cap, "]");
...
int ccwld_plan_seal(ccwld_plan *p, ccwld_error *e) {
    ...
    char *s = NULL;
    size_t slen = 0;
    if (!ccwld_plan_serialize(p, &s, &slen, e)) return 0;
    free(p->serialized);
    p->serialized = s;
...
int ccwld_plan_hash(const ccwld_plan *p, char out[65]) {
    ...
    char *tmp = NULL;
    if (!s) {
        if (!ccwld_plan_serialize(p, &tmp, &slen, NULL)) return 0;
        s = tmp;
    }
...
ccwld_expr *ccwld_expr_binary(char op, ccwld_expr *a, ccwld_expr *b) {
    return ccwld_expr_binary((ccwld_op_tag)op, a, b);
}

ccwld_expr *ccwld_expr_unary(char op, ccwld_expr *a) {
    return ccwld_expr_unary((ccwld_op_tag)op, a);
}

ccwld_expr *ccwld_expr_align_expr(ccwld_expr *a, uint64_t boundary) {
...
int ccwld_expr_eval(const ccwld_expr *e, const ccwld_plan *p,
