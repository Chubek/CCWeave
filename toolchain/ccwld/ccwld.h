...
extern "C"
{
  ...
  int ccwld_plan_input (ccwld_plan *, const char *, int as_needed, int startup,
                        ccwld_error *);
  int ccwld_plan_search_path (ccwld_plan *, const char *, ccwld_error *);
  int ccwld_plan_memory (ccwld_plan *, const char *, const char *, uint64_t,
                         uint64_t, ccwld_error *);
  int ccwld_plan_section (ccwld_plan *, const char *, const char *, uint64_t,
                          const char *, const char *, ccwld_error *);
  int ccwld_plan_symbol (ccwld_plan *, const char *, ccwld_expr *, int provide,
                         int hidden, ccwld_error *);
  int ccwld_plan_hook (ccwld_plan *, ccwld_phase,
                       int (*) (ccwld_phase, ccwld_link *, void *), void *,
                       ccwld_error *);
  ...
  int ccwld_expr_eval (const ccwld_expr *, const ccwld_plan *, uint64_t dot,
                       uint64_t *out, ccwld_error *);
  int ccwld_plan_seal (ccwld_plan *, ccwld_error *);
  int ccwld_plan_serialize (const ccwld_plan *, char **out, size_t *len,
                            ccwld_error *);
  int ccwld_plan_hash (const ccwld_plan *, char out[65]);
  int ccwld_link_run (ccwld_plan *, const char *output, ccwld_error *);
  /* Link an ordered list of object/archive paths without invoking a
   * subprocess. */
