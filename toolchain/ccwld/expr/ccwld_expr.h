 ...
 * Evaluation happens during layout in plan order against a live location counter. */
#ifndef CCWLD_EXPR_H
#define CCWLD_EXPR_H
#include <stdint.h>
#include <stddef.h>
...
typedef enum {
    CCWLD_OP_ADD = '+', CCWLD_OP_SUB = '-', CCWLD_OP_MUL = '*',
    CCWLD_OP_DIV = '/', CCWLD_OP_MOD = '%',
    CCWLD_OP_AND = '&', CCWLD_OP_OR  = '|', CCWLD_OP_XOR = '^',
    CCWLD_OP_SHL = '<', CCWLD_OP_SHR = '>',
    CCWLD_OP_NEG = '~', CCWLD_OP_NOT = '!',
    CCWLD_OP_ABS = 'A',
} ccwld_op_tag;
