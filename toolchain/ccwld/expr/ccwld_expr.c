...
ccwld_expr *
ccwld_expr_binary (ccwld_op_tag op, ccwld_expr *a, ccwld_expr *b)
{
  ccwld_expr *e = calloc (1, sizeof (*e));
  if (!e)
    return NULL;
...
ccwld_expr *
ccwld_expr_unary (ccwld_op_tag op, ccwld_expr *a)
{
  ccwld_expr *e = calloc (1, sizeof (*e));
  if (!e)
    return NULL;
...
ccwld_expr *
ccwld_expr_align (ccwld_expr *a, uint64_t boundary)
{
  ccwld_expr *e = calloc (1, sizeof (*e));
  if (!e)
    return NULL;
...
ccwld_expr *
ccwld_expr_min (ccwld_expr *a, ccwld_expr *b)
{
  ccwld_expr *e = calloc (1, sizeof (*e));
  if (!e)
    return NULL;
...
ccwld_expr *
ccwld_expr_cond (ccwld_expr *test, ccwld_expr *then_e, ccwld_expr *else_e)
{
  ccwld_expr *e = calloc (1, sizeof (*e));
  if (!e)
    return NULL;
...
ccwld_expr *
ccwld_expr_defined (const char *symbol_name)
{
  ccwld_expr *e = calloc (1, sizeof (*e));
  if (!e)
    return NULL;
...
ccwld_expr *
ccwld_expr_region_origin (const char *region_name)
{
  ccwld_expr *e = calloc (1, sizeof (*e));
  if (!e)
    return NULL;
...
ccwld_expr *
ccwld_expr_region_length (const char *region_name)
{
  ccwld_expr *e = calloc (1, sizeof (*e));
  if (!e)
    return NULL;
...
ccwld_expr *
ccwld_expr_sizeof (const char *section_name)
{
  ccwld_expr *e = calloc (1, sizeof (*e));
  if (!e)
    return NULL;
...
ccwld_expr *
ccwld_expr_addr (const char *section_name)
{
  ccwld_expr *e = calloc (1, sizeof (*e));
  if (!e)
    return NULL;
...
ccwld_expr *
ccwld_expr_loadaddr (const char *section_name)
{
  ccwld_expr *e = calloc (1, sizeof (*e));
  if (!e)
    return NULL;
...
ccwld_expr *
ccwld_expr_sizeof_headers (void)
{
  ccwld_expr *e = calloc (1, sizeof (*e));
  if (!e)
    return NULL;
...
ccwld_expr *
ccwld_expr_segment_start (const char *segment_name)
{
  ccwld_expr *e = calloc (1, sizeof (*e));
  if (!e)
    return NULL;
...
ccwld_expr *
ccwld_expr_clone (const ccwld_expr *e)
{
  if (!e)
    return NULL;
...
int
ccwld_expr_equal (const ccwld_expr *a, const ccwld_expr *b)
{
  if (!a && !b)
    return 1;
  if (!a || !b)
  ...
  if (!ccwld_expr_equal (a->a, b->a))
    return 0;
  if (!ccwld_expr_equal (a->b, b->b))
    return 0;
  if (!ccwld_expr_equal (a->c, b->c))
    return 0;
  return 1;
}
...
static int eval_inner (const ccwld_expr *e, const struct ccwld_plan *plan,
                       uint64_t dot, uint64_t *out, char **error_message);

static int
eval_inner (const ccwld_expr *e, const struct ccwld_plan *plan, uint64_t dot,
            uint64_t *out, char **error_message)
{
  if (!e)
    {
      if (error_message)
  ...
  switch (e->kind)
    ...
    case CCWLD_EXPR_REGION_ORIGIN:
      {
        const ccwld_mem *r = plan_find_region (plan, e->name);
        if (!r)
          {
            if (error_message)
              {
    ...
    case CCWLD_EXPR_SIZEOF:
      {
        const ccwld_sec *s = plan_find_section (plan, e->name);
        if (!s)
          {
            if (error_message)
              {
                char buf[256];
                snprintf (buf, sizeof (buf), "undefined section '%s'",
                          e->name);
                *error_message = strdup (buf);
              }
            ok = 0;
    ...
    case CCWLD_EXPR_ALIGN:
      if (!eval_inner (e->a, plan, dot, &a_val, error_message))
        {
          ok = 0;
          break;
        }
    ...
    case CCWLD_EXPR_UNARY:
      ...
      switch (e->op)
        {
        case CCWLD_OP_NEG:
          *out = (uint64_t)(-(int64_t)a_val);
          break;
        case CCWLD_OP_NOT:
          *out = !a_val;
          break;
        case CCWLD_OP_ABS:
          *out = (int64_t)a_val < 0 ? (uint64_t)(-(int64_t)a_val) : a_val;
          break;
        default:
          if (error_message)
            {
              char buf[256];
              snprintf (buf, sizeof (buf), "unknown unary operator '%c'",
                        (char)e->op);
              *error_message = strdup (buf);
            }
          ok = 0;
    ...
    case CCWLD_EXPR_BINARY:
      ...
      if (!eval_inner (e->b, plan, dot, &b_val, error_message))
        {
          ok = 0;
          break;
        }
      switch (e->op)
        ...
        case CCWLD_OP_DIV:
          if (b_val == 0)
            {
              if (error_message)
                *error_message = strdup ("division by zero");
              ok = 0;
            }
          else
            {
        ...
        case CCWLD_OP_MOD:
          if (b_val == 0)
            {
              if (error_message)
                *error_message = strdup ("modulo by zero");
              ok = 0;
            }
          else
            {
        ...
        default:
          if (error_message)
            {
              char buf[256];
              snprintf (buf, sizeof (buf), "unknown binary operator '%c'",
                        (char)e->op);
              *error_message = strdup (buf);
            }
          ok = 0;
...
int
ccwld_expr_eval (const ccwld_expr *e, const struct ccwld_plan *plan,
                 uint64_t dot, uint64_t *out, char **error_message)
{
  if (!e || !plan || !out)
    {
      if (error_message)
        *error_message = strdup ("invalid arguments to eval");
      return 0;
    }
...
void
ccwld_expr_to_string (const ccwld_expr *e, char **out, size_t *len,
                      size_t *cap)
{
  if (!e)
    {
      APPEND_BUF (*out, *len, *cap, "null");
      return;
    }
