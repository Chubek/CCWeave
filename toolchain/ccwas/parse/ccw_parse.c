...
static char *strip_comment(char *s, int is_wasm) {
...
static const char *x86_regs[] = {
  ...
  "rip","xmm0","xmm1","xmm2","xmm3","xmm4","xmm5","xmm6","xmm7",
...
int ccw_parse_mem(ccw_arch_t arch, const char *s, ccw_mem_t *mem, char **error) {
  ...
  if (*p != '[') {
    if (error) *error = strdup("expected '[' in memory operand");
    return 0;
  }
  p++;
  ...
  char tok[128];
  int tok_idx = 0;
  int state = 0; /* 0 = base, 1 = after +/-, 2 = index, 3 = after *, 4 = scale */
  char saved_base[128] = "";
  ...
  while (*p && *p != ']') {
    ...
    if (*p == '+' || *p == '-') {
      if (tok_idx > 0) {
        tok[tok_idx] = '\0';
        if (state == 0) { strcpy(saved_base, tok); }
        else if (state == 2) { strcpy(saved_index, tok); }
        else if (state == 4) {
          saved_disp = (sign == '-') ? -strtoll(tok, NULL, 0) : strtoll(tok, NULL, 0);
        }
        tok_idx = 0;
      }
      sign = *p;
  ...
  if (*p != ']') {
    if (error) *error = strdup("unterminated memory operand: expected ']'");
    return 0;
  }

  /* Fill in mem struct */
  if (saved_base[0]) mem->base = strdup(saved_base);
  if (saved_index[0]) mem->index = strdup(saved_index);
  mem->disp = saved_disp;
...
static int parse_one_operand(ccw_arch_t arch, const char *s, ccw_operand_t *op,
                             char **error) {
    ...
    if (tok[0] == '\'') {
      ...
      if (tok[1] == '\\' && tok[2] && tok[3] == '\'') {
        switch (tok[2]) {
          case 'n': op->imm = '\n'; break;
          case 't': op->imm = '\t'; break;
          case '0': op->imm = '\0'; break;
          default: op->imm = (int64_t)tok[2]; break;
        }
      } else if (tok[1] && tok[2] == '\'') {
...
int ccw_parse_operands(ccw_arch_t arch, const char *s, ccw_operand_t **out,
  ...
  *count = 0;
  ...
  if (!ops) { if (error) *error = strdup("out of memory"); return 0; }
  ...
  while (token && idx < n) {
    ...
    if (!parse_one_operand(arch, t, &ops[idx], error)) {
      for (size_t j = 0; j < idx; j++) ccw_operand_free(&ops[j]);
      free(ops);
      free(buf);
      return 0;
    }
...
int ccw_parse_line(const char *line, ccw_stmt_t *stmt, ccw_arch_t arch,
  ...
  /* Skip empty */
  if (!line || !*line) { stmt->kind = CCW_STMT_EMPTY; return 0; }
  ...
  if (!*p) { free(buf); stmt->kind = CCW_STMT_EMPTY; return 0; }
  ...
  if (colon) {
    ...
    char *pre = p;
    while (pre < colon && isspace((unsigned char)*pre)) pre++;
    ...
    if (!strcmp(dir, "text")) {
      ...
      if (error) {
        ...
        snprintf(ebuf, sizeof(ebuf),
                 "unsupported directive alias '.%s'; use the canonical CCWAS name",
                 dir);
        *error = strdup(ebuf);
      }
      free(buf);
  ...
  for (char *c = mnemonic; *c; c++) *c = (char)tolower((unsigned char)*c);
  ...
  /* Parse operands */
  if (*args) {
    ccw_parse_operands(arch, args, &stmt->insn.operands, &stmt->insn.op_count, error);
  } else {
...
int ccw_parse_asm(ccw_unit_t *u, const char *source, const char *filename,
  ...
  while (line) {
    ...
    if (!*p) { free(ln); line = strtok_r(NULL, "\n", &saveptr); continue; }
    ...
    if (ret < 0) {
      if (error) {
        char ebuf[512];
        snprintf(ebuf, sizeof(ebuf), "%s:%d: %s", filename, line_num, parse_err);
        *error = strdup(ebuf);
      }
      free(parse_err);
      free(ln);
    ...
    if (ret > 0) {
      switch (stmt.kind) {
        case CCW_STMT_LABEL: {
          ...
          uint64_t off = 0;
          if (u->cur_section >= 0 && (size_t)u->cur_section < kv_size(u->sections)) {
            off = kv_A(u->sections, u->cur_section).len;
          }
          ccw_symbol_t *existing = ccw_symtab_lookup(u->symtab, stmt.label);
          if (existing && existing->defined) {
            if (error) {
              char ebuf[512];
              snprintf(ebuf, sizeof(ebuf), "%s:%d: symbol '%s' redefined",
                       filename, line_num, stmt.label);
              *error = strdup(ebuf);
            }
            free(stmt.label);
            free(ln);
          ...
          ccw_symtab_define(u->symtab, stmt.label, off, u->cur_section, binding);
          free(stmt.label);
          break;
        }
          ...
          case CCW_DIR_BSS:
            ...
            case CCW_DIR_SECTION: {
              ...
              for (size_t i = 0; i < kv_size(u->sections); i++) {
                if (!strcmp(kv_A(u->sections, i).name, stmt.dir.str_val)) {
                  found = (int)i; break;
                }
              }
              if (found < 0) {
                found = ccw_unit_add_section(u, stmt.dir.str_val, 1,
            ...
            case CCW_DIR_GLOBAL: {
              ccw_symbol_t *sym = ccw_symtab_lookup(u->symtab, stmt.dir.str_val);
              if (sym) sym->binding = 1;
              else {
                sym = ccw_symtab_define(u->symtab, stmt.dir.str_val, 0, -1, 1);
                sym->defined = 0;
              }
              free(stmt.dir.str_val);
              break;
            ...
            case CCW_DIR_ALIGN: {
              if (u->cur_section >= 0) {
                ccw_section_t *sec = &kv_A(u->sections, u->cur_section);
                uint64_t align = (uint64_t)stmt.dir.int_val;
                if (stmt.dir.int_val <= 0 || (align & (align - 1)) != 0) {
                  if (error) {
                    char ebuf[512];
                    snprintf(ebuf, sizeof(ebuf),
                             "%s:%d: alignment must be a positive power of two",
                             filename, line_num);
                    *error = strdup(ebuf);
                  }
                  free(ln);
                ...
                uint8_t fill = (u->arch == CCW_ARCH_X86_64 && (sec->flags & 4)) ? 0x90 : 0;
                while (sec->len & mask) ccw_unit_emit_byte(u, u->cur_section, fill);
              }
              break;
            }
            case CCW_DIR_BYTE:
            ...
            case CCW_DIR_8BYTE: {
              int width = (stmt.dir.kind == CCW_DIR_BYTE) ? 1 :
                          ...
                          (stmt.dir.kind == CCW_DIR_4BYTE) ? 4 : 8;
              for (size_t i = 0; i < kv_size(stmt.dir.data); i++) {
                ...
                if (dv->label) {
                  ...
                  ccw_reloc_type_t rtype = (width == 8) ? CCW_RELOC_ABS64 : CCW_RELOC_ABS32;
                  ccw_unit_emit_reloc(u, rtype, off, 0, dv->label, -1);
                  for (int j = 0; j < width; j++) ccw_unit_emit_byte(u, u->cur_section, 0);
                } else {
                  for (int j = 0; j < width; j++) {
                    ccw_unit_emit_byte(u, u->cur_section, (uint8_t)(dv->value & 0xFF));
                    dv->value >>= 8;
                  }
                }
                free(dv->label);
              ...
              if (stmt.dir.str_val) {
                ...
                for (size_t i = 0; i < len; i++) {
                  if (s[i] == '\\' && i + 1 < len) {
                    i++;
                    switch (s[i]) {
                      case 'n': ccw_unit_emit_byte(u, u->cur_section, '\n'); break;
                      case 't': ccw_unit_emit_byte(u, u->cur_section, '\t'); break;
                      case 'r': ccw_unit_emit_byte(u, u->cur_section, '\r'); break;
                      case '0': ccw_unit_emit_byte(u, u->cur_section, '\0'); break;
                      case '\\': ccw_unit_emit_byte(u, u->cur_section, '\\'); break;
                      default: ccw_unit_emit_byte(u, u->cur_section, s[i]); break;
                    }
                  } else {
                    ccw_unit_emit_byte(u, u->cur_section, (uint8_t)s[i]);
                  }
                }
                free(stmt.dir.str_val);
              }
            ...
            case CCW_DIR_EQU: {
              ...
              if (eq) {
                *eq = '\0'; eq++;
                ...
                ccw_symbol_t *existing = ccw_symtab_lookup(u->symtab, stmt.dir.str_val);
                if (existing && existing->defined) {
                  if (error) {
                    char ebuf[512];
                    snprintf(ebuf, sizeof(ebuf), "%s:%d: symbol '%s' redefined",
                             filename, line_num, stmt.dir.str_val);
                    *error = strdup(ebuf);
                  }
                  free(stmt.dir.str_val);
                  free(ln);
                ...
                ccw_symtab_define(u->symtab, stmt.dir.str_val, (uint64_t)val, -1, 0);
              }
              free(stmt.dir.str_val);
              break;
            }
        ...
        case CCW_STMT_INSN: {
          ...
          if (!in_macro) {
            ...
            /* Validate ISA first */
            ccw_form_t form;
            if (!ccw_isa_validate(u, stmt.insn.mnemonic, &form, &enc_err)) {
              if (error) {
                char ebuf[512];
                snprintf(ebuf, sizeof(ebuf), "%s:%d: %s", filename, line_num, enc_err);
                *error = strdup(ebuf);
              }
              free(enc_err);
              ccw_stmt_free(&stmt);
