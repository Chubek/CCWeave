...
{
    if (error) *error = message ? strdup(message) : NULL;
    return 0;
}

static char *read_all(const char *path, char **error)
{
    ...
    while (!feof(f)) {
        ...
        if (n && kputsn(buf, (int)n, &out) == EOF) {
            fclose(f); free(out.s); fail(error, "out of memory"); return NULL;
        }
        if (ferror(f)) {
            char msg[512];
            snprintf(msg, sizeof(msg), "%s: %s", path, strerror(errno));
            fclose(f); free(out.s); fail(error, msg); return NULL;
        }
    }
    fclose(f);
...
int ccwas_assemble(const char *source, const char *filename,
                   ...
                   char **error)
{
    ccwas_options defaults = {CCW_ARCH_X86_64, "intel", CCW_FMT_ELF,
                              NULL, 0, 0, 0, 0};
    ...
    if (error) *error = NULL;
    if (!source || !output_path) return fail(error, "source and output_path are required");
    ...
    ccw_lccwas_init(&lua,
                    ...
                    o.arch == CCW_ARCH_RISCV64 ? "riscv64" : "wasm32",
    ...
    for (size_t i = 0; i < o.define_count; ++i) {
        ...
        if (eq) {
            ...
            if (!key || !ccw_lccwas_define(&lua, key, eq + 1)) {
                free(key); ccw_lccwas_destroy(&lua); return fail(error, "out of memory");
            }
            free(key);
        } else if (!ccw_lccwas_define(&lua, def, "1")) {
            ccw_lccwas_destroy(&lua); return fail(error, "out of memory");
        }
    }
    ...
    if (!ccw_parse_asm(&unit, expanded, filename, &err)) {
        if (error) *error = err; else free(err);
        free(expanded); ccw_unit_destroy(&unit); ccw_lccwas_destroy(&lua); return 0;
    }
    if (unit.error_count || (o.werror && unit.warning_count)) {
        ...
        free(expanded); ccw_unit_destroy(&unit); ccw_lccwas_destroy(&lua);
...
void ccwas_free_error(char *error) { free(error); }
