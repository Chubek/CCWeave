...
static uint32_t
reloc_to_elf_type (ccw_reloc_type_t t, ccw_arch_t arch)
{
  switch (arch)
    {
    case CCW_ARCH_X86_64:
      switch (t)
        {
        case CCW_RELOC_ABS64:
          return R_X86_64_64;
        case CCW_RELOC_PC32:
        case CCW_RELOC_REL32:
          return R_X86_64_PC32;
        case CCW_RELOC_ABS32:
          return R_X86_64_32;
        case CCW_RELOC_REL16:
    ...
    case CCW_ARCH_RISCV64:
      switch (t)
        ...
        case CCW_RELOC_LO12_I:
          return R_RISCV_PCREL_LO12_I;
        case CCW_RELOC_LO12_S:
...
static Elf64_Half
arch_to_emachine (ccw_arch_t arch)
{
  switch (arch)
    {
    case CCW_ARCH_X86_64:
...
static size_t
build_strtab (char *buf, size_t cap, const char **strings, size_t n,
              size_t *offsets)
{
...
/* Write ELF64 object */
static int
write_elf64 (const ccw_unit_t *u, const char *path, char **error)
{
  (void)error;
  ...
  for (size_t i = 1; i <= nsym; i++)
    strtab_cap += strlen (sym_names[i]) + 1;
  ...
  size_t shnum
      = 1 + nsec + nrela
        + 4; /* NULL + sections + rela + symtab + strtab + shstrtab + note */
  ...
  for (size_t i = 0; i < kv_size (u->relocs); i++)
    {
      ccw_reloc_t *r = &kv_A (u->relocs, i);
      size_t sidx = (r->section >= 0 && (size_t)r->section < nrela)
                        ? (size_t)r->section
                        : 0;
      rela_counts[sidx]++;
    }
  ...
  for (size_t i = 0; i < nsec; i++)
    {
      if (kv_A (u->sections, i).data && kv_A (u->sections, i).len > 0)
        {
          memcpy (buf + sec_offsets[i], kv_A (u->sections, i).data,
                  kv_A (u->sections, i).len);
        }
    }
          ...
          if (r->symbol)
            {
              for (size_t s = 1; s <= nsym; s++)
                {
                  if (!strcmp (sym_names[s], r->symbol))
                    {
                      sym_idx = s;
                      break;
                    }
  ...
  if (!f)
    {
      ...
      free (strtab);
      if (error)
        *error = strdup ("cannot open output file");
      return 0;
    }
...
static int
write_pecoff (const ccw_unit_t *u, const char *path, char **error)
{
  (void)u;
  (void)path;
  if (error)
    *error = strdup ("PE/COFF output not yet implemented");
  return 0;
}

/* Mach-O writer (stub for now) */
static int
write_macho (const ccw_unit_t *u, const char *path, char **error)
{
  (void)u;
  (void)path;
  if (error)
    *error = strdup ("Mach-O output not yet implemented");
  return 0;
}

/* Main dispatch */
int
ccw_obj_write (const ccw_unit_t *u, const char *path, ccw_obj_format_t fmt,
               char **error)
{
  switch (fmt)
    {
    case CCW_FMT_ELF:
      return write_elf64 (u, path, error);
    case CCW_FMT_PE:
      return write_pecoff (u, path, error);
    case CCW_FMT_MACHO:
      return write_macho (u, path, error);
    default:
      if (error)
        *error = strdup ("unsupported output format");
      return 0;
    }
}

ccw_obj_format_t
ccw_obj_default_format (void)
{
