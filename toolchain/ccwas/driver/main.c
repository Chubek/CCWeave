...
static char *
read_file (const char *path, char **error)
{
...
static ccw_obj_format_t
parse_format (const char *f)
{
  if (!f)
    return ccw_obj_default_format ();
  if (!strcmp (f, "elf"))
    return CCW_FMT_ELF;
  if (!strcmp (f, "pe"))
    return CCW_FMT_PE;
...
main (int argc, char *argv[])
{
  ...
  ko_longopt_t longopts[] = { { "target", ko_required_argument, 't' },
  ...
  while ((c = ketopt (&s, argc, argv, 1, "D:o:W:I:f:", longopts)) != -1)
    {
      switch (c)
        ...
        case 'o':
          ...
          break;
        case 'W':
          opts.werror = (s.arg && !strcmp (s.arg, "error")) ? 1 : 0;
          break;
        case 'f':
          opts.format = s.arg;
          break;
        case 'h':
          ...
          printf (
              "usage: ccwas --target=<arch> [options] input.s -o output.o\n");
          printf ("  --target=<arch>  Required. x86-64, aarch64, riscv64, "
                  "wasm32\n");
          ...
          printf (
              "  --unsafe-lua     Enable unsafe Lua (for local dev only)\n");
  ...
  if (!opts.target)
    die ("--target is required (x86-64, aarch64, riscv64, wasm32)");
  if (!opts.input)
    die ("no input file specified");
  if (!opts.output)
    die ("-o output.o is required");
