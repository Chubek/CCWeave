...
extern "C"
{
  ...
  int ccw_unit_add_section (ccw_unit_t *u, const char *name, int type,
                            uint64_t flags);
  void ccw_unit_emit_byte (ccw_unit_t *u, int sidx, uint8_t b);
  void ccw_unit_emit_bytes (ccw_unit_t *u, int sidx, const uint8_t *data,
                            size_t len);
  void ccw_unit_emit_reloc (ccw_unit_t *u, ccw_reloc_type_t type,
                            uint64_t offset, uint64_t addend,
                            const char *symbol, int section);

  /* --- ISA validation --- */
  int ccw_isa_validate (ccw_unit_t *u, const char *mnemonic,
                        ccw_form_t *form_out, char **error);
