...
void ccw_symtab_destroy(khash_t(ccw_sym) *tab) {
  ...
  for (k = kh_begin(tab); k != kh_end(tab); ++k) {
    if (kh_exist(tab, k)) free((char *)kh_key(tab, k));
  }
...
static void skip_ws(void) {
  while (expr_pos < expr_end && isspace((unsigned char)*expr_pos)) expr_pos++;
}

static ccw_expr_t *expr_new(ccw_expr_kind_t kind) {
...
static ccw_expr_t *expr_primary(void) {
  ...
  if (expr_pos >= expr_end) {
    snprintf(expr_err_buf, sizeof(expr_err_buf), "unexpected end of expression");
  ...
  if (*expr_pos == '\'') {
    expr_pos++;
    if (expr_pos >= expr_end) { expr_err = "unterminated character literal"; return NULL; }
    char c;
    if (*expr_pos == '\\') {
      expr_pos++;
      if (expr_pos >= expr_end) { expr_err = "unterminated character literal"; return NULL; }
    ...
    if (expr_pos >= expr_end || *expr_pos != '\'') {
      expr_err = "unterminated character literal"; return NULL;
    }
    expr_pos++;
  ...
  if (*expr_pos == '.' && (expr_pos + 1 >= expr_end || !isalnum((unsigned char)expr_pos[1]))) {
    expr_pos++;
    ccw_expr_t *e = expr_new(CCW_EXPR_CURRENT);
    return e;
  }
  /* identifier */
  if (isalpha((unsigned char)*expr_pos) || *expr_pos == '_' || *expr_pos == '.') {
    const char *start = expr_pos;
    while (expr_pos < expr_end && (isalnum((unsigned char)*expr_pos) ||
           *expr_pos == '_' || *expr_pos == '.')) expr_pos++;
  ...
  snprintf(expr_err_buf, sizeof(expr_err_buf), "unexpected '%c' in expression", *expr_pos);
  expr_err = expr_err_buf;
  return NULL;
}
...
static ccw_expr_t *expr_unary(void) {
  skip_ws();
  if (expr_pos < expr_end && (*expr_pos == '-' || *expr_pos == '~' || *expr_pos == '!')) {
    char op = *expr_pos;
    expr_pos++;
    ...
    e->unary.op = (op == '-') ? CCW_EXPR_OP_NEG :
                  (op == '~') ? CCW_EXPR_OP_COM : CCW_EXPR_OP_NOT;
    e->unary.child = child;
    return e;
  }
  return expr_primary();
...
static ccw_expr_t *expr_mul(void) {
  ...
  while (1) {
    skip_ws();
    if (expr_pos >= expr_end) break;
    ccw_expr_op_t op;
    switch (*expr_pos) {
      case '*': op = CCW_EXPR_OP_MUL; break;
      case '/': op = CCW_EXPR_OP_DIV; break;
      case '%': op = CCW_EXPR_OP_MOD; break;
      default: return left;
    }
    expr_pos++;
    ccw_expr_t *right = expr_unary();
    if (!right) { ccw_expr_free(left); return NULL; }
    ccw_expr_t *e = expr_new(CCW_EXPR_BINARY);
    e->binary.op = op; e->binary.left = left; e->binary.right = right;
    left = e;
  }
  return left;
}
...
static ccw_expr_t *expr_add(void) {
  ...
  while (1) {
    ...
    ccw_expr_op_t op;
    switch (*expr_pos) {
      case '+': op = CCW_EXPR_OP_ADD; break;
      case '-': op = CCW_EXPR_OP_SUB; break;
      default: return left;
    }
    expr_pos++;
...
static ccw_expr_t *expr_shift(void) {
  ...
  while (1) {
    skip_ws();
    if (expr_pos + 1 >= expr_end) break;
    ccw_expr_op_t op;
    if (*expr_pos == '<' && expr_pos[1] == '<') { op = CCW_EXPR_OP_SHL; }
    else if (*expr_pos == '>' && expr_pos[1] == '>') { op = CCW_EXPR_OP_SHR; }
    else return left;
    expr_pos += 2;
...
static ccw_expr_t *expr_bitand(void) {
  ...
  while (1) {
    skip_ws();
    if (expr_pos >= expr_end || *expr_pos != '&') return left;
    expr_pos++;
    ccw_expr_t *right = expr_shift();
    if (!right) { ccw_expr_free(left); return NULL; }
    ccw_expr_t *e = expr_new(CCW_EXPR_BINARY);
    e->binary.op = CCW_EXPR_OP_AND; e->binary.left = left; e->binary.right = right;
    left = e;
  }
}
...
static ccw_expr_t *expr_bitxor(void) {
  ...
  while (1) {
    ...
    if (!right) { ccw_expr_free(left); return NULL; }
    ccw_expr_t *e = expr_new(CCW_EXPR_BINARY);
    e->binary.op = CCW_EXPR_OP_XOR; e->binary.left = left; e->binary.right = right;
    left = e;
  }
}
...
static ccw_expr_t *expr_bitor(void) {
  ...
  while (1) {
    ...
    if (!right) { ccw_expr_free(left); return NULL; }
    ccw_expr_t *e = expr_new(CCW_EXPR_BINARY);
    e->binary.op = CCW_EXPR_OP_OR; e->binary.left = left; e->binary.right = right;
    left = e;
  }
}

static ccw_expr_t *expr_parse_inner(void) { return expr_bitor(); }
...
int ccw_expr_parse(const char *s, ccw_expr_t **out, char **error) {
  ...
  if (expr_pos < expr_end) {
    /* trailing garbage */
    if (error) {
      snprintf(expr_err_buf, sizeof(expr_err_buf), "trailing characters: '%s'", expr_pos);
...
int64_t ccw_expr_eval(const ccw_expr_t *e, const khash_t(ccw_sym) *symtab,
  ...
  switch (e->kind) {
    ...
    case CCW_EXPR_CURRENT:
      *ok = 1; return (int64_t)pc;
    case CCW_EXPR_SYM: {
      if (!symtab) { *ok = 0; return 0; }
      ccw_symbol_t *sym = ccw_symtab_lookup((khash_t(ccw_sym) *)symtab, e->sval);
      if (!sym || !sym->defined) { *ok = 0; return 0; }
      *ok = 1; return (int64_t)sym->value;
    }
    case CCW_EXPR_UNARY: {
      int64_t v = ccw_expr_eval(e->unary.child, symtab, pc, ok);
      if (!*ok) return 0;
      switch (e->unary.op) {
        case CCW_EXPR_OP_NEG: return -v;
        case CCW_EXPR_OP_NOT: return !v ? 1 : 0;
        case CCW_EXPR_OP_COM: return ~v;
    ...
    case CCW_EXPR_BINARY: {
      ...
      if (!*ok) return 0;
      switch (e->binary.op) {
        case CCW_EXPR_OP_ADD: return l + r;
        case CCW_EXPR_OP_SUB: return l - r;
        case CCW_EXPR_OP_MUL: return l * r;
        case CCW_EXPR_OP_DIV: if (r == 0) { *ok = 0; return 0; } return l / r;
        case CCW_EXPR_OP_MOD: if (r == 0) { *ok = 0; return 0; } return l % r;
        case CCW_EXPR_OP_SHL: return l << (r & 63);
        case CCW_EXPR_OP_SHR: return l >> (r & 63);
        case CCW_EXPR_OP_AND: return l & r;
        case CCW_EXPR_OP_OR:  return l | r;
        case CCW_EXPR_OP_XOR: return l ^ r;
        default: *ok = 0; return 0;
      }
    }
  }
...
void ccw_expr_free(ccw_expr_t *e) {
  if (!e) return;
  switch (e->kind) {
    case CCW_EXPR_SYM: free(e->sval); break;
    case CCW_EXPR_UNARY: ccw_expr_free(e->unary.child); break;
    case CCW_EXPR_BINARY:
...
void ccw_operand_free(ccw_operand_t *op) {
  if (!op) return;
  switch (op->kind) {
    case CCW_OP_REG: free(op->reg); break;
    case CCW_OP_MEM:
      free(op->mem.base); free(op->mem.index); free(op->mem.seg);
      break;
    case CCW_OP_LABEL: free(op->label); break;
    case CCW_OP_EXPR: ccw_expr_free((ccw_expr_t *)op->expr); break;
    case CCW_OP_REG_LIST:
      for (size_t i = 0; i < op->reg_list.count; i++) free(op->reg_list.regs[i]);
...
ccw_operand_t ccw_operand_copy(const ccw_operand_t *src) {
  ...
  switch (src->kind) {
    case CCW_OP_REG: dst.reg = src->reg ? strdup(src->reg) : NULL; break;
    case CCW_OP_LABEL: dst.label = src->label ? strdup(src->label) : NULL; break;
    case CCW_OP_MEM:
...
void ccw_stmt_free(ccw_stmt_t *s) {
  ...
  switch (s->kind) {
    case CCW_STMT_INSN:
      for (size_t i = 0; i < s->insn.op_count; i++) ccw_operand_free(&s->insn.operands[i]);
    ...
    case CCW_STMT_DIRECTIVE:
      switch (s->dir.kind) {
        case CCW_DIR_SECTION: case CCW_DIR_INCLUDE: free(s->dir.str_val); break;
        case CCW_DIR_BYTE: case CCW_DIR_2BYTE: case CCW_DIR_4BYTE: case CCW_DIR_8BYTE:
        case CCW_DIR_ASCII: case CCW_DIR_ASCIZ: case CCW_DIR_ZERO: case CCW_DIR_SPACE:
      ...
      break;
    case CCW_STMT_LABEL: free(s->label); break;
    default: break;
  }
}
...
void ccw_unit_init(ccw_unit_t *u, ccw_arch_t arch, const char *syntax) {
  ...
  u->text_section = ccw_unit_add_section(u, ".text", 1 /* SHT_PROGBITS */,
                                         2 /* SHF_ALLOC */ | 4 /* SHF_EXECINSTR */);
  u->data_section = ccw_unit_add_section(u, ".data", 1, 2 /* SHF_ALLOC */ | 1 /* SHF_WRITE */);
...
void ccw_unit_emit_byte(ccw_unit_t *u, int sidx, uint8_t b) {
  if (sidx < 0 || (size_t)sidx >= kv_size(u->sections)) return;
...
void ccw_unit_emit_reloc(ccw_unit_t *u, ccw_reloc_type_t type, uint64_t offset,
  ...
  ccw_reloc_t r = { type, offset, addend, symbol ? strdup(symbol) : NULL, section };
...
int ccw_isa_validate(ccw_unit_t *u, const char *mnemonic, ccw_form_t *form_out,
  ...
  if (u->arch != CCW_ARCH_X86_64) {
    ...
    if (form_out) *form_out = CCW_FORM_CUSTOM;
    return 1;
  }
  for (const ccw_isa_entry_t *e = x86_64_isa; e->name; e++) {
    if (!strcmp(e->name, mnemonic)) {
      if (e->extension && !ccw_ext_is_enabled(u, e->extension)) {
        if (error) {
          char buf[256];
          snprintf(buf, sizeof(buf), "instruction '%s' requires extension '%s'",
                   mnemonic, e->extension);
          *error = strdup(buf);
        }
        return 0;
      }
      if (form_out) *form_out = e->form;
      return 1;
    }
  }
  if (error) {
    char buf[256];
    snprintf(buf, sizeof(buf), "unknown instruction '%s' for target", mnemonic);
    *error = strdup(buf);
  }
  return 0;
}
...
int ccw_ext_enable(ccw_unit_t *u, const char *ext) {
  ...
  for (int i = 0; i < u->enabled_extensions.count; i++) {
    if (!strcmp(u->enabled_extensions.names[i], ext)) return 1;
  }
  if (u->enabled_extensions.count >= u->enabled_extensions.cap) {
    int newcap = u->enabled_extensions.cap ? u->enabled_extensions.cap * 2 : 4;
    char **tmp = (char **)realloc(u->enabled_extensions.names, (size_t)newcap * sizeof(char *));
    if (!tmp) return 0;
  ...
  for (int i = 0; i < u->enabled_extensions.count; i++) {
    if (!strcmp(u->enabled_extensions.names[i], ext)) {
      ...
      memmove(&u->enabled_extensions.names[i],
              ...
              (size_t)(u->enabled_extensions.count - i - 1) * sizeof(char *));
