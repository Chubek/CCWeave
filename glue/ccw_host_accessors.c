...
static bool arg_node(const ccw_val *v, ccw_node *out)
{
    if (v->type == CCW_T_NODE) { *out = v->as.node; return true; }
    if (v->type == CCW_T_INT && v->as.i >= 0) { *out = (ccw_node)v->as.i; return true; }
    return false;
}

static bool arg_index(const ccw_val *v, int *out)
{
    if (v->type != CCW_T_INT) return false;
    *out = (int)v->as.i;
    return true;
}

static bool arg_symbol(const ccw_val *v, const char **out)
{
    if (v->type != CCW_T_SYMBOL && v->type != CCW_T_STRING) return false;
    *out = v->as.s;
    return true;
}
...
#define ACC_UNUSED (void)host_ctx; (void)args; (void)nargs;

/* ---------- reflection ---------- */

ACC_SIG(acc_glue_has)
{
    (void)host_ctx; (void)ir;
    ...
    static const char *const core[] = {
        "glue-has?", "ir-profile", "ir-function-count", "ir-function-ref",
        "function-name", "function-block-count", "function-block-ref",
    ...
    for (size_t i = 0; i < sizeof(core) / sizeof(core[0]); i++)
        if (strcmp(core[i], name) == 0) { found = true; break; }
...
ACC_SIG(acc_ir_function_ref)
{
    ...
    if (fn == 0) return acc_fail(error_message, "function index out of range");
...
ACC_SIG(acc_function_name)
{
    ...
    if (name == NULL) return acc_fail(error_message, "not a function node");
...
ACC_SIG(acc_function_block_ref)
{
    ...
    if (blk == 0) return acc_fail(error_message, "block index out of range");
...
ACC_SIG(acc_block_instr_ref)
{
    ...
    if (ins == 0) return acc_fail(error_message, "instruction index out of range");
...
ACC_SIG(acc_instr_opcode)
{
    ...
    if (op == NULL) return acc_fail(error_message, "not an instruction node");
...
ACC_SIG(acc_instr_operand)
{
    ...
    if (nargs != 2 || !arg_node(&args[0], &ins) || !arg_index(&args[1], &idx))
        return acc_fail(error_message, "expects an instruction node and an index");
    ccw_node opnd = ccw_ir_instr_operand(ir, ins, idx);
    if (opnd == 0) return acc_fail(error_message, "operand index out of range");
...
ACC_SIG(acc_instr_build)
{
    ...
    if (nargs < 1 || !arg_symbol(&args[0], &opcode))
        return acc_fail(error_message, "expects an opcode symbol and operand nodes");
    ...
    for (int i = 1; i < nargs; i++) {
        ...
        if (i == 1) {
            ...
            if (t != CCW_TY_VOID) type = t;
        }
    }

    ccw_node ins = ccw_ir_instr_build(ir, opcode, type);
    if (ins == 0) return acc_fail(error_message, "could not build instruction");
    for (int i = 1; i < nargs; i++) {
        ccw_node opnd = 0;
...
ACC_SIG(acc_const_int_build)
{
    ...
    if (n == 0) return acc_fail(error_message, "could not build constant");
...
static bool edit_allowed(ccw_ir *ir, ccw_edit_kind kind,
                         ccw_node target, ccw_node incoming)
{
    if (g_edit_hook == NULL) return true;
...
ACC_SIG(acc_instr_replace)
{
    ...
    if (nargs != 2 || !arg_node(&args[0], &old_ins) || !arg_node(&args[1], &new_ins))
    ...
    if (ccw_ir_instr_replace(ir, old_ins, new_ins) != CCW_OK)
        return acc_fail(error_message, "replacement failed: check attachment state");
    *result = ccw_nil();
    return CCW_OK;
}
...
ACC_SIG(acc_instr_insert_before)
{
    ...
    if (nargs != 2 || !arg_node(&args[0], &anchor) || !arg_node(&args[1], &new_ins))
        return acc_fail(error_message, "expects two instruction nodes");
    if (!edit_allowed(ir, CCW_EDIT_INSERT_BEFORE, anchor, new_ins))
        return acc_fail(error_message, "edit rejected by host");
    if (ccw_ir_instr_insert_before(ir, anchor, new_ins) != CCW_OK)
        return acc_fail(error_message, "insertion failed: check attachment state");
    *result = ccw_nil();
    return CCW_OK;
}
...
ACC_SIG(acc_instr_delete)
{
    ...
    if (ccw_ir_instr_delete(ir, ins) != CCW_OK)
        return acc_fail(error_message, "deletion failed: instruction is not attached");
    *result = ccw_nil();
    return CCW_OK;
}
...
{
    struct { const char *name; int min; int max; ccw_accessor_fn fn; } table[] = {
