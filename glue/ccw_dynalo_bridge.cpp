...
extern "C" void *ccw_dynalo_open(const char *path, const char **error_message)
{
    ...
    try {
        ...
        if (error_message) *error_message = ccw_dynalo_error.c_str();
        return nullptr;
    }
}

extern "C" void *ccw_dynalo_symbol(void *library, const char *name,
...
extern "C" void ccw_dynalo_close(void *library)
{
    if (!library) return;
    try {
        dynalo::close(reinterpret_cast<dynalo::native::handle>(library));
    } catch (...) {
        /* Destruction paths cannot report an exception through the C ABI. */
    }
}
