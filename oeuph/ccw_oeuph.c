...
static int
eg_add_node (egraph *g, const enode *n)
{
  /* Hash-consing by structural identity keeps congruence closure cheap. */
  for (int i = 0; i < g->count; i++)
    {
      const enode *e = &g->nodes[i];
      if (e->is_const != n->is_const)
        continue;
      if (strcmp (e->opcode, n->opcode) != 0)
        continue;
      if (strcmp (e->reg, n->reg) != 0)
        continue;
      if (e->is_const && e->value != n->value)
        continue;
      if (e->nchildren != n->nchildren)
        continue;
      bool same = true;
  ...
  if (g->count == g->cap)
    {
      if (!reserve_items ((void **)&g->nodes, &g->cap, g->count + 1,
                          sizeof (*g->nodes))
          || !reserve_items ((void **)&g->parent, &g->cap, g->count + 1,
                             sizeof (*g->parent)))
        return -1;
    }
  int id = g->count++;
...
eg_free (egraph *g)
{
  kvec_t (enode) nodes = { (size_t)g->count, (size_t)g->cap, g->nodes };
  kvec_t (int) parent = { (size_t)g->count, (size_t)g->cap, g->parent };
  kv_destroy (nodes);
  kv_destroy (parent);
  memset (g, 0, sizeof (*g));
}
          ...
          for (int j = i + 1; j < g->count; j++)
            {
              ...
              if (a->is_const != b->is_const)
                continue;
              if (strcmp (a->opcode, b->opcode) != 0)
                continue;
              if (strcmp (a->reg, b->reg) != 0)
                continue;
              if (a->is_const && a->value != b->value)
                continue;
...
ccw_oeuph_ruleset *
ccw_oeuph_ruleset_create (const char *name)
{
...
void
ccw_oeuph_ruleset_destroy (ccw_oeuph_ruleset *rs)
{
  if (rs == NULL)
    return;
  kvec_t (ccw_rule) rules = { (size_t)rs->count, (size_t)rs->cap, rs->rules };
  kv_destroy (rules);
  free (rs);
}

const char *
ccw_oeuph_ruleset_name (const ccw_oeuph_ruleset *rs)
{
  return rs ? rs->name : "";
}

int
ccw_oeuph_ruleset_size (const ccw_oeuph_ruleset *rs)
{
  return rs ? rs->count : 0;
}

static ccw_status
rule_error (char **error_message, const char *fmt, ...)
{
...
ccw_status
ccw_oeuph_rule_add (ccw_oeuph_ruleset *rs, const char *rule_name,
                    ...
                    char **error_message)
{
  ...
  if (!reserve_items ((void **)&rs->rules, &rs->cap, rs->count + 1,
                      sizeof (*rs->rules)))
    return CCW_ERR_OOM;
...
static binding *
binding_find (bindings *b, const char *name)
{
...
static bool
binding_add_class (bindings *b, const char *name, int eclass)
{
  binding *found = binding_find (b, name);
  if (found != NULL)
    return !found->is_value && found->eclass == eclass;
  if (b->count >= CCW_PAT_MAX_NODES)
    return false;
...
static bool
binding_add_value (bindings *b, const char *name, int64_t value)
{
  binding *found = binding_find (b, name);
  if (found != NULL)
    return found->is_value && found->value == value;
  if (b->count >= CCW_PAT_MAX_NODES)
    return false;
  binding *slot = &b->items[b->count++];
  snprintf (slot->name, sizeof (slot->name), "%s", name);
...
static bool
match_node (egraph *g, const ccw_pattern *pat, int pidx, int nidx, bindings *b)
{
  ...
  if (n->is_const || strcmp (n->opcode, p->text) != 0)
    return false;
  if (n->nchildren != p->nchildren)
    return false;
      ...
      for (int j = 0; j < g->count && !matched; j++)
        {
          if (eg_find (g, j) != want)
            continue;
          bindings saved = *b;
          if (match_node (g, pat, p->children[i], j, b))
            matched = true;
          else
            *b = saved;
        }
...
static int
instantiate (egraph *g, const ccw_pattern *pat, int pidx, bindings *b)
{
  const ccw_pat_node *p = &pat->nodes[pidx];
  enode n;
  memset (&n, 0, sizeof (n));
    ...
    case CCW_PAT_VAR:
      {
        ...
        if (bind == NULL || bind->is_value)
          return -1;
        /* Return any representative e-node of the bound class. */
        for (int i = 0; i < g->count; i++)
          if (eg_find (g, i) == bind->eclass)
            return i;
        return -1;
      }
    case CCW_PAT_CONST:
...
static int
import_operand (egraph *g, const ccw_ir *ir, ccw_node opnd)
{
  enode n;
  memset (&n, 0, sizeof (n));
  int64_t value = 0;
...
static int
import_instr (egraph *g, const ccw_ir *ir, ccw_node ins)
{
  enode n;
  memset (&n, 0, sizeof (n));
...
static int
node_cost (const enode *n, ccw_cost_model model)
{
  if (n->is_const || n->opcode[0] == '\0')
    return 1;
  ...
  if (strcmp (n->opcode, "imul") == 0)
    return 30;
  if (strcmp (n->opcode, "idiv") == 0)
    return 40;
  if (strcmp (n->opcode, "shl") == 0 || strcmp (n->opcode, "shr") == 0)
    return 4;
  if (strcmp (n->opcode, "iadd") == 0 || strcmp (n->opcode, "isub") == 0)
    return 5;
  if (strcmp (n->opcode, "imov") == 0)
    return 2;
  return 10;
}
...
static void
extract (egraph *g, ccw_cost_model model, int *best_node, int *best_cost)
{
  for (int i = 0; i < g->count; i++)
    {
      best_node[i] = -1;
      best_cost[i] = INT32_MAX;
    }
      ...
      for (int i = 0; i < g->count; i++)
        {
          ...
          if (!ready)
            continue;
          int cls = eg_find (g, i);
          if (cost < best_cost[cls]
              || (cost == best_cost[cls] && i < best_node[cls]))
            {
              best_cost[cls] = cost;
              best_node[cls] = i;
              changed = true;
...
static bool
writeback_instr (ccw_ir *ir, ccw_node ins, egraph *g, int enode_idx)
{
      ...
      for (int i = 0; i < n->nchildren && operands_match; i++)
        {
          ...
          if (child->is_const)
            operands_match
                = ccw_ir_const_int_value (ir, opnd, &value) == CCW_OK
                  && value == child->value;
          else
            {
              const char *name = ccw_ir_operand_name (ir, opnd);
  ...
  ccw_node built
      = ccw_ir_instr_build (ir, n->opcode, ccw_ir_instr_type (ir, ins));
  if (built == 0)
    return false;
  const char *dest = ccw_ir_instr_dest (ir, ins);
  if (dest != NULL)
    ccw_ir_instr_set_dest (ir, built, dest);
...
ccw_status
ccw_oeuph_run (ccw_ir *ir, const ccw_oeuph_ruleset *rs,
                  ...
                  for (int ri = 0; ri < rs->count; ri++)
                    {
                      ...
                      for (int dir = 0; dir < (rule->bidirectional ? 2 : 1);
                           dir++)
                        {
                          const ccw_pattern *from
                              = dir == 0 ? &rule->lhs : &rule->rhs;
                          const ccw_pattern *to
                              = dir == 0 ? &rule->rhs : &rule->lhs;
                          int snapshot = g.count;
                          for (int ni = 0; ni < snapshot; ni++)
                            {
                              bindings b;
                              b.count = 0;
                              if (!match_node (&g, from, from->root, ni, &b))
                                continue;
                              local.matches++;
                  ...
                  if (g.count > budget.max_nodes)
                    {
                      ...
                      snprintf (local.budget_hit, sizeof (local.budget_hit),
                                "nodes");
                      break;
                    }
              ...
              if (have_best)
                {
                  ...
                  if (chosen >= 0)
                    writeback_instr (ir, ins, &g, chosen);
                }
