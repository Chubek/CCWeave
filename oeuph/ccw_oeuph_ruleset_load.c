...
static void skip_blanks_and_comments(const char **p)
{
    for (;;) {
...
static char *read_form(const char **p)
{
...
static char *next_item(const char **p)
{
    skip_blanks_and_comments(p);
...
ccw_oeuph_ruleset *ccw_oeuph_ruleset_load(const char *path, char **error_message)
{
    ...
    if (text == NULL) { set_err(error_message, "cannot read ruleset file"); return NULL; }
    ...
    while ((form = read_form(&cursor)) != NULL) {
        ...
        if (head == NULL) { free(form); continue; }

        if (strcmp(head, "ruleset") == 0) {
            char *name = next_item(&inner);
            if (name == NULL) {
                set_err(error_message, "ruleset form needs a name");
                free(head); free(form); free(text);
                ccw_oeuph_ruleset_destroy(rs);
                return NULL;
            }
            if (rs == NULL) rs = ccw_oeuph_ruleset_create(name);
            free(name);
        } else if (strcmp(head, "rule") == 0) {
            if (rs == NULL) {
                set_err(error_message, "rule appears before the ruleset declaration");
                free(head); free(form); free(text);
                return NULL;
            }
            char *name = next_item(&inner);
            ...
            while ((opt = next_item(&inner)) != NULL) {
                if (strcmp(opt, ":bidirectional") == 0) {
                    ...
                    if (value != NULL && strcmp(value, "#f") == 0) bidirectional = false;
                    free(value);
                }
                free(opt);
            }
            if (name != NULL && lhs != NULL && rhs != NULL) {
                ...
                if (ccw_oeuph_rule_add(rs, name, lhs, rhs, bidirectional, NULL,
                                       &rule_err) != CCW_OK) {
                    if (error_message) *error_message = rule_err;
                    else free(rule_err);
                    free(name); free(lhs); free(rhs); free(head); free(form); free(text);
                    ccw_oeuph_ruleset_destroy(rs);
                    return NULL;
                }
            }
            free(name); free(lhs); free(rhs);
        }
        free(head);
        free(form);
    }
    ...
    if (rs == NULL) set_err(error_message, "file declares no ruleset");
    return rs;
}
