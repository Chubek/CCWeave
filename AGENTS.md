...
# Notice for GLM 5.3 and Kimi 3

## Shell execution is broken for kimi-3 / glm-5.3 — do not use Bash

**This section applies to you if you are running as `kimi-3` or `glm-5.3`, or if any
tool call of yours returns the error shown below.**

The Bash tool in this environment is broken. Every invocation fails immediately with:

```
private bwrap shim refused the invocation: the secure Claude bwrap layout did not contain the expected Bash wrapper
```

This is a harness-level sandbox failure. The command never runs — retrying, rephrasing,
or picking a different command cannot help.

### Rules

1. **Never call the Bash tool.** Not for `ls`, not for `echo`, not "just to check".
   One failed attempt maximum; on seeing the bwrap error, stop invoking Bash for the
   rest of the session.
2. **Use dedicated file tools instead of shell equivalents.** These bypass the broken
   sandbox and work normally:
   - Inspect: `Read`, `Glob`, `Grep` — never `cat`/`head`/`tail`/`find`/`grep` via shell
   - Modify: `Write`, `Edit`, `NotebookEdit` — never `sed`/`awk`/redirects
3. **If something genuinely must run as a command, delegate it.** Spawn a Claude Code
   subagent/task with:
   - the exact command(s) in a copy-pasteable block,
   - a request for verbatim stdout/stderr and the exit code,
   - an explicit instruction that if *it* also hits the bwrap error, it should say so
     and stop — not retry.
4. **If delegation also fails, ask the user.** Present all needed commands in one
