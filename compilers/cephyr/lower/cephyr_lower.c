...
{
    if (!ct) return CCW_TY_VOID;
    switch (ct->kind) {
    case CEPHYR_TY_VOID:   return CCW_TY_VOID;
    case CEPHYR_TY_BOOL:   return CCW_TY_I1;
    case CEPHYR_TY_CHAR:
    case CEPHYR_TY_SCHAR:
    case CEPHYR_TY_UCHAR:  return CCW_TY_I8;
    case CEPHYR_TY_SHORT:
    case CEPHYR_TY_USHORT: return CCW_TY_I16;
    case CEPHYR_TY_INT:
    case CEPHYR_TY_UINT:   return CCW_TY_I32;
    case CEPHYR_TY_LONG:
    case CEPHYR_TY_ULONG:
    case CEPHYR_TY_LONGLONG:
    case CEPHYR_TY_ULONGLONG: return CCW_TY_I64;
    case CEPHYR_TY_FLOAT:  return CCW_TY_F32;
    case CEPHYR_TY_DOUBLE:
    case CEPHYR_TY_LONGDOUBLE: return CCW_TY_F64;
    case CEPHYR_TY_PTR:    return CCW_TY_PTR;
    case CEPHYR_TY_ARRAY:  return CCW_TY_PTR; /* arrays decay to pointers */
    case CEPHYR_TY_ENUM:   return CCW_TY_I32;
    case CEPHYR_TY_STRUCT:
    case CEPHYR_TY_UNION:  return CCW_TY_PTR; /* structs are passed by pointer */
    case CEPHYR_TY_FUNC:   return CCW_TY_PTR; /* function pointers */
    case CEPHYR_TY_TYPEDEF: return ct->inner ? cephyr_lower_map_type(ct->inner) : CCW_TY_VOID;
    default: return CCW_TY_VOID;
    }
}
...
static char *fresh_label(cephyr_lower_ctx *ctx, const char *prefix)
{
    char *buf = malloc(64);
...
static char *lower_binary(cephyr_lower_ctx *ctx, ccw_ir *ir, ccw_node blk,
                          const cephyr_ast_node *node)
{
...
static char *lower_unary(cephyr_lower_ctx *ctx, ccw_ir *ir, ccw_node blk,
                         const cephyr_ast_node *node)
{
...
static char *lower_call(cephyr_lower_ctx *ctx, ccw_ir *ir, ccw_node blk,
                        const cephyr_ast_node *node)
{
    /* Lower args */
    ...
    if (node->data.call.callee->kind == CEPHYR_NODE_EXPR_IDENT && node->data.call.callee->name) {
...
static void lower_block(cephyr_lower_ctx *ctx, ccw_ir *ir, ccw_node fn,
                        const cephyr_ast_node *node, ccw_node *current_blk)
{
    if (!node || node->kind != CEPHYR_NODE_BLOCK) return;
                       ...
                       const cephyr_ast_node *node, ccw_node *current_blk)
{
    ...
    case CEPHYR_NODE_STMT_RETURN: {
        if (node->data.unop.operand) {
            char *val = lower_expr(ctx, ir, *current_blk, node->data.unop.operand);
    ...
    case CEPHYR_NODE_STMT_IF: {
        char *cond = lower_expr(ctx, ir, *current_blk, node->data.if_stmt.cond);
        ...
        *current_blk = then_blk;
        ...
        if (*current_blk) ccw_kliche_jump(ir, *current_blk, end_lbl);

        *current_blk = else_blk;
        if (node->data.if_stmt.else_branch) {
            lower_stmt(ctx, ir, fn, node->data.if_stmt.else_branch, current_blk);
        }
        if (*current_blk) ccw_kliche_jump(ir, *current_blk, end_lbl);

        *current_blk = end_blk;
        free(then_lbl); free(else_lbl); free(end_lbl);
        break;
    }
    ...
    case CEPHYR_NODE_STMT_WHILE: {
        ...
        *current_blk = cond_blk;
        ...
        char *cond = lower_expr(ctx, ir, *current_blk, node->data.while_stmt.cond);
        ...
        *current_blk = body_blk;
        ...
        if (*current_blk) ccw_kliche_jump(ir, *current_blk, cond_lbl);

        *current_blk = end_blk;
        free(cond_lbl); free(body_lbl); free(end_lbl);
        break;
    }
                               ...
                               char **error_message)
{
    ...
    if (func_def->type && func_def->type->param_count > 0) {
        for (int i = 0; i < func_def->type->param_count; i++) {
            ccw_ir_type pty = cephyr_lower_map_type(func_def->type->param_types[i]);
            const char *pname = (func_def->type->param_names && func_def->type->param_names[i])
