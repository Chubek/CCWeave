...
cephyr_ast_node *
cephyr_ast_node_alloc (cephyr_ast_node_kind kind)
{
...
void
cephyr_ast_node_free (cephyr_ast_node *node)
{
  if (!node)
    return;
  free ((void *)node->name);
...
void
cephyr_ast_free_recursive (cephyr_ast_node *node)
{
  if (!node)
    return;
  /* Free siblings first */
...
static const char *
kind_name (cephyr_ast_node_kind k)
{
  switch (k)
    {
...
static void
dump_type (FILE *out, const cephyr_type *t)
{
  if (!t)
    {
      fputs ("(null)", out);
  ...
  const char *names[]
      = { "void",  "char",   "schar",   "uchar", "short",  "ushort",
          "int",   "uint",   "long",    "ulong", "llong",  "ullong",
          "float", "double", "ldouble", "_Bool", "ptr",    "array",
...
void
cephyr_ast_dump (FILE *out, const cephyr_ast_node *root)
{
  if (!out || !root)
    return;
