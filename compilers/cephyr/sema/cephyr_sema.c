...
static void
add_diag (cephyr_sema_ctx *ctx, cephyr_severity sev, const char *id,
          const char *msg, const cephyr_ast_node *node)
{
  if (ctx->diag_count >= CEPHYR_MAX_DIAGNOSTICS)
    return;
...
scope_free (scope *s)
{
  ...
  if (s->symbols)
    {
      for (khint_t i = kh_begin (s->symbols); i != kh_end (s->symbols); ++i)
        {
...
static void
scope_insert (scope *s, const char *name, cephyr_ast_node *node,
              cephyr_type *type)
{
...
bool
cephyr_type_is_integer (const cephyr_type *t)
{
  if (!t)
    return false;
  switch (t->kind)
...
bool
cephyr_type_is_float (const cephyr_type *t)
{
  if (!t)
    return false;
...
bool
cephyr_type_is_arithmetic (const cephyr_type *t)
{
  return cephyr_type_is_integer (t) || cephyr_type_is_float (t);
}

bool
cephyr_type_is_scalar (const cephyr_type *t)
{
  return cephyr_type_is_arithmetic (t) || (t && t->kind == CEPHYR_TY_PTR);
}

bool
cephyr_type_compatible (const cephyr_type *a, const cephyr_type *b)
{
  if (!a || !b)
    return false;
  /* Same kind */
  ...
  if (a->kind == CEPHYR_TY_PTR)
    {
      if (a->inner && a->inner->kind == CEPHYR_TY_VOID)
        return true;
      if (b->inner && b->inner->kind == CEPHYR_TY_VOID)
        return true;
...
cephyr_type *
cephyr_type_integer_promote (cephyr_type *t)
{
  if (!t)
    return NULL;
  if (!cephyr_type_is_integer (t))
    return t;
...
cephyr_type *
cephyr_type_usual_arithmetic (cephyr_type *a, cephyr_type *b)
{
  if (!a || !b)
    return NULL;
...
size_t
cephyr_type_size (const cephyr_type *t)
{
  if (!t)
    return 0;
  switch (t->kind)
    ...
    case CEPHYR_TY_UNION:
      {
        /* Sum fields (struct) or max field (union), simplified to 8-aligned
         * sum */
        size_t total = 0;
    ...
    case CEPHYR_TY_TYPEDEF:
      return t->inner ? cephyr_type_size (t->inner) : 0;
    default:
      return 0;
    }
}
...
size_t
cephyr_type_align (const cephyr_type *t)
{
  if (!t)
    return 1;
...
cephyr_type *
cephyr_type_dup (const cephyr_type *t)
{
  if (!t)
    return NULL;
  ...
  if (t->inner)
    copy->inner = cephyr_type_dup (t->inner);
  if (t->return_type)
    copy->return_type = cephyr_type_dup (t->return_type);
  if (t->name)
    copy->name = cephyr_strdup (t->name);
  /* Deep copy arrays */
  if (t->field_count > 0)
    {
      copy->field_names = calloc ((size_t)t->field_count, sizeof (char *));
      copy->field_types
          = calloc ((size_t)t->field_count, sizeof (cephyr_type *));
      copy->field_bit_widths = calloc ((size_t)t->field_count, sizeof (int));
      for (int i = 0; i < t->field_count; i++)
        {
          if (t->field_names[i])
            copy->field_names[i] = cephyr_strdup (t->field_names[i]);
...
void
cephyr_type_free (cephyr_type *t)
{
  if (!t)
    return;
  free ((void *)t->name);
...
void
cephyr_sema_destroy (cephyr_sema_ctx *ctx)
{
  if (!ctx)
    return;
...
static void
declare_func (cephyr_sema_ctx *ctx, cephyr_ast_node *node)
{
  if (!node->name)
    return;
  ...
  if (existing)
    {
      add_diag (ctx, CEPHYR_SEV_ERROR, CEPHYR_E0004, "redefinition of symbol",
                node);
      return;
    }
...
static void
declare_var (cephyr_sema_ctx *ctx, cephyr_ast_node *node)
{
  if (!node->name)
    return;
  symbol_entry *existing = scope_lookup (ctx->current_scope, node->name);
...
static void
declare_node (cephyr_sema_ctx *ctx, cephyr_ast_node *node)
{
  if (!node)
    return;
  switch (node->kind)
...
int
cephyr_sema_declare (cephyr_sema_ctx *ctx, cephyr_ast_node *program)
{
  ctx->error_count = 0;
  ctx->diag_count = 0;
...
static cephyr_type *
resolve_type_ref (cephyr_sema_ctx *ctx, cephyr_type *t)
{
  if (!t)
    return NULL;
...
static void
resolve_node_type (cephyr_sema_ctx *ctx, cephyr_ast_node *node)
{
  if (!node)
    return;
...
int
cephyr_sema_resolve_types (cephyr_sema_ctx *ctx, cephyr_ast_node *program)
{
  ctx->error_count = 0;
  ctx->diag_count = 0;
...
static void
check_expr (cephyr_sema_ctx *ctx, cephyr_ast_node *node)
{
  if (!node)
    return;
  switch (node->kind)
    ...
    case CEPHYR_NODE_EXPR_CONDITIONAL:
      ...
      if (node->data.if_stmt.then_branch
          && node->data.if_stmt.then_branch->type)
        {
          ...
          if (st->kind == CEPHYR_TY_STRUCT || st->kind == CEPHYR_TY_UNION)
            {
              for (int i = 0; i < st->field_count; i++)
                {
                  if (st->field_names[i]
                      && strcmp (st->field_names[i], node->data.member.member)
                             == 0)
                    {
...
static void
check_stmt (cephyr_sema_ctx *ctx, cephyr_ast_node *node)
{
  if (!node)
    return;
  switch (node->kind)
...
static void
check_node (cephyr_sema_ctx *ctx, cephyr_ast_node *node)
{
  if (!node)
    return;
...
int
cephyr_sema_check (cephyr_sema_ctx *ctx, cephyr_ast_node *program)
{
  ctx->error_count = 0;
  ctx->diag_count = 0;
...
int
cephyr_sema_run (cephyr_sema_ctx *ctx, cephyr_ast_node *program)
{
...
int
cephyr_sema_diagnostic_count (const cephyr_sema_ctx *ctx)
{
  return ctx ? ctx->diag_count : 0;
}

const cephyr_diagnostic *
cephyr_sema_diagnostic_ref (const cephyr_sema_ctx *ctx, int idx)
{
  if (!ctx || idx < 0 || idx >= ctx->diag_count)
    return NULL;
  return &ctx->diagnostics[idx];
}

void
cephyr_sema_diagnostic_emit (FILE *out, const cephyr_diagnostic *diag)
{
  if (!out || !diag)
    return;
  ...
  switch (diag->severity)
    {
    case CEPHYR_SEV_WARNING:
      sev_str = "warning";
      break;
    case CEPHYR_SEV_ERROR:
      sev_str = "error";
      break;
    case CEPHYR_SEV_FATAL:
      sev_str = "fatal error";
      break;
    default:
      break;
  ...
  if (diag->source_file && diag->source_line > 0)
    {
      fprintf (out, "%s:%d:%d: %s: %s [%s]\n", diag->source_file,
               diag->source_line, diag->source_column, sev_str, diag->message,
               diag->id);
    }
  else
    {
...
int
cephyr_sema_error_count (const cephyr_sema_ctx *ctx)
{
