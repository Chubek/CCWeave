...
{
    return cephyr_cpp_preprocess_with_defines(
        source_text, source_len, source_name, include_paths,
    ...
    const char *const *defines, int define_count)
{
    return cephyr_cpp_preprocess_with_options(
        source_text, source_len, source_name, include_paths,
        include_path_count, defines, define_count, NULL, 0, NULL, 0);
}
    ...
    const char *const *args, int arg_count)
{
        ...
        for (int i = 0; i < item_count; ++i) {
            ...
            if (item == NULL) {
                result.error_message = cephyr_cpp_strdup(
                    "invalid ucpp preprocessor option");
                goto option_error;
            }
            if (!strncmp(item, "-D", 2) && item[2] != '\0') {
                const char **next = (const char **)realloc(
                    extra_defines, (size_t)(extra_define_count + 1) *
                    sizeof(*next));
                if (next == NULL) goto option_oom;
                extra_defines = next;
                ...
                const char **next = (const char **)realloc(
                    extra_includes, (size_t)(extra_include_count + 1) *
                    sizeof(*next));
                if (next == NULL) goto option_oom;
                extra_includes = next;
    ...
    if (total_include_count > 0) {
        ...
        for (int i = 0; i < extra_include_count; i++)
            ucpp_include_paths[include_path_count + i] =
                (char *)extra_includes[i];
    }
    ...
    if (undef_special) no_special_macros = 1;
...
static int append_shell_word(char **command, size_t *length, size_t *capacity,
                             const char *word)
{
    size_t needed = 3u;
    char *next;
...
char *cephyr_cpp_external(const char *source_path,
                          ...
                          char **error_message)
{
    return cephyr_cpp_external_with_options(source_path, cpp_command,
                                            NULL, 0, NULL, 0,
                                            error_message);
}
