...
static const char *gnu_attr_names[] = { "noreturn",
                                        "const",
                                        "pure",
                                        "malloc",
                                        "format",
...
static bool
gnu_attr_handler (void *ast_node, const char *attr_name,
                  const char *const *attr_args)
{
  ...
  if (strcmp (attr_name, "noreturn") == 0)
    return true;
  if (strcmp (attr_name, "const") == 0)
    return true;
  if (strcmp (attr_name, "pure") == 0)
    return true;
  if (strcmp (attr_name, "malloc") == 0)
    return true;
  return true; /* recognized, no error */
}

static cephyr_module *
gnu_attributes_init (void)
{
...
const cephyr_module_info *
cephyr_module_get_info (const cephyr_module *mod)
{
  return mod ? &mod->info : NULL;
}

int
cephyr_module_fragment_count (const cephyr_module *mod)
{
  return mod ? mod->fragment_count : 0;
}

const cephyr_module_fragment *
cephyr_module_fragment_ref (const cephyr_module *mod, int idx)
{
  if (!mod || idx < 0 || idx >= mod->fragment_count)
    return NULL;
  return &mod->fragments[idx];
}

int
cephyr_module_attr_handler_count (const cephyr_module *mod)
{
  return mod ? mod->attr_count : 0;
}

const char *
cephyr_module_attr_name (const cephyr_module *mod, int idx)
{
  if (!mod || idx < 0 || idx >= mod->attr_count)
    return NULL;
  return mod->attr_names[idx];
}

cephyr_attr_handler_fn
cephyr_module_attr_handler (const cephyr_module *mod, int idx)
{
  if (!mod || idx < 0 || idx >= mod->attr_count)
    return NULL;
...
const cephyr_module *
cephyr_stdmodule_gnu_attributes (void)
{
  static cephyr_module *mod = NULL;
  if (!mod)
    mod = gnu_attributes_init ();
  return mod;
}

const cephyr_module *
cephyr_stdmodule_lto (void)
{
  static cephyr_module *mod = NULL;
  if (!mod)
    mod = lto_init ();
