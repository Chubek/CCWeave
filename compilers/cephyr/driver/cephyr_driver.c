...
static char *cephyr_driver_strdup(const char *s)
{
    ...
    if (s == NULL || kputs(s, &copy) == EOF) return NULL;
                              ...
                              const char ***out, int *out_count)
{
    ...
    *out_count = 0;
    if (total == 0) return 1;
    if (kv_resize(const char *, merged, total) == NULL) return 0;
    for (size_t i = 0; i < first_count; ++i) merged.a[i] = first[i];
