...
static char *emit_target_assembly(const ccw_ir *ir, const char *triple)
{
    const char *arch = cephyr_target_arch(triple);
    size_t cap = 256, len = 0;
    char *out;
    int functions;
    if (ir == NULL || arch == NULL) return NULL;
    out = (char *)malloc(cap);
    if (out == NULL) return NULL;
    out[0] = '\0';
#define APPEND_FMT(...) do { \
        char _tmp[256]; int _n = snprintf(_tmp, sizeof(_tmp), __VA_ARGS__); \
        if (_n < 0) { free(out); return NULL; } \
        if (len + (size_t)_n + 1u > cap) { \
            size_t _newcap = cap; while (_newcap < len + (size_t)_n + 1u) _newcap *= 2u; \
            char *_grown = (char *)realloc(out, _newcap); \
            if (_grown == NULL) { free(out); return NULL; } \
            out = _grown; cap = _newcap; \
        } \
        memcpy(out + len, _tmp, (size_t)_n); len += (size_t)_n; out[len] = '\0'; \
    } while (0)
                                 ...
                                 const cephyr_options *opts)
{
    ...
    if (command == NULL || arch == NULL || assembly_path == NULL ||
        object_path == NULL || cmd == NULL) {
        free(cmd);
        return -1;
    }
#define CMD_APPEND(...) do { \
        char _tmp[512]; int _n = snprintf(_tmp, sizeof(_tmp), __VA_ARGS__); \
        if (_n < 0) { free(cmd); return -1; } \
        if (len + (size_t)_n + 1u > cap) { \
            while (cap < len + (size_t)_n + 1u) cap *= 2u; \
            char *_grown = (char *)realloc(cmd, cap); \
            if (_grown == NULL) { free(cmd); return -1; } \
            cmd = _grown; \
        } \
        memcpy(cmd + len, _tmp, (size_t)_n); len += (size_t)_n; cmd[len] = '\0'; \
    } while (0)
...
static cephyr_result cephyr_compile_inner(const cephyr_options *opts)
{
    ...
    if (cephyr_target_arch(opts->target_triple) == NULL) {
        fprintf(stderr, "cephyr: unsupported target triple '%s' (use --lstriples)\n",
    ...
    if (result == CEPHYR_SUCCESS && !opts->emit_ir) {
        ...
        if (assembly == NULL) {
            ...
            if (opts->output_path != NULL && strcmp(opts->output_path, "-") != 0) {
            ...
            if (!opts->keep_temp && assembly_path != NULL) unlink(assembly_path);
            if ((opts->output_path == NULL || strcmp(opts->output_path, "-") == 0) &&
                !opts->keep_temp && object_path != NULL) unlink(object_path);
        }
        free(assembly_path);
        free(object_path);
        free(assembly);
