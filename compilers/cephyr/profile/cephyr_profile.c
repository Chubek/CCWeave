...
static void profile_error(char *out, size_t capacity, const char *fmt, ...)
{
    va_list ap;
...
static int profile_push_kernel(cephyr_profile *profile,
                               ...
                               const char *prefer)
{
    ...
    if ((name != NULL && !(item->name = profile_strdup(name))) ||
        (capability != NULL && !(item->capability = profile_strdup(capability))) ||
...
static const cyaml_schema_field_t yaml_kernel_fields[] = {
    CYAML_FIELD_STRING_PTR("name", CYAML_FLAG_OPTIONAL,
                           yaml_kernel, name, 0, CYAML_UNLIMITED),
    CYAML_FIELD_STRING_PTR("capability", CYAML_FLAG_OPTIONAL,
                           yaml_kernel, capability, 0, CYAML_UNLIMITED),
    CYAML_FIELD_STRING_PTR("prefer", CYAML_FLAG_OPTIONAL,
                           yaml_kernel, prefer, 0, CYAML_UNLIMITED),
    CYAML_FIELD_END
};
...
static const cyaml_schema_field_t yaml_command_fields[] = {
    CYAML_FIELD_STRING_PTR("name", CYAML_FLAG_DEFAULT,
                           yaml_command, name, 0, CYAML_UNLIMITED),
    CYAML_FIELD_STRING_PTR("command", CYAML_FLAG_DEFAULT,
                           yaml_command, command, 0, CYAML_UNLIMITED),
    CYAML_FIELD_END
};

static const cyaml_schema_value_t yaml_command_schema = {
    CYAML_VALUE_MAPPING(CYAML_FLAG_DEFAULT, yaml_command,
                        yaml_command_fields),
};
...
static const cyaml_schema_field_t yaml_profile_fields[] = {
    ...
    CYAML_FIELD_STRING_PTR("name", CYAML_FLAG_OPTIONAL,
                           yaml_profile, name, 0, CYAML_UNLIMITED),
    CYAML_FIELD_STRING_PTR("target", CYAML_FLAG_OPTIONAL,
                           yaml_profile, target, 0, CYAML_UNLIMITED),
    CYAML_FIELD_STRING_PTR("opt_level", CYAML_FLAG_OPTIONAL,
                           yaml_profile, opt_level, 0, CYAML_UNLIMITED),
    CYAML_FIELD_STRING_PTR("preprocessor", CYAML_FLAG_OPTIONAL,
                           yaml_profile, preprocessor, 0, CYAML_UNLIMITED),
    CYAML_FIELD_STRING_PTR("manifest_dir", CYAML_FLAG_OPTIONAL,
                           yaml_profile, manifest_dir, 0, CYAML_UNLIMITED),
    CYAML_FIELD_STRING_PTR("sched_script", CYAML_FLAG_OPTIONAL,
                           yaml_profile, sched_script, 0, CYAML_UNLIMITED),
    CYAML_FIELD_STRING_PTR("assembler", CYAML_FLAG_OPTIONAL,
                           yaml_profile, assembler, 0, CYAML_UNLIMITED),
    CYAML_FIELD_STRING_PTR("linker", CYAML_FLAG_OPTIONAL,
                           yaml_profile, linker, 0, CYAML_UNLIMITED),
    CYAML_FIELD_SEQUENCE("include_paths", CYAML_FLAG_POINTER | CYAML_FLAG_OPTIONAL,
                         yaml_profile, include_paths, &yaml_string_schema,
                         0, CYAML_UNLIMITED),
    CYAML_FIELD_SEQUENCE("defines", CYAML_FLAG_POINTER | CYAML_FLAG_OPTIONAL,
                         yaml_profile, defines, &yaml_string_schema,
                         0, CYAML_UNLIMITED),
    CYAML_FIELD_SEQUENCE("preprocessor_options",
                         CYAML_FLAG_POINTER | CYAML_FLAG_OPTIONAL,
                         yaml_profile, preprocessor_options,
                         &yaml_string_schema, 0, CYAML_UNLIMITED),
    CYAML_FIELD_SEQUENCE("preprocessor_args",
                         CYAML_FLAG_POINTER | CYAML_FLAG_OPTIONAL,
                         yaml_profile, preprocessor_args,
                         &yaml_string_schema, 0, CYAML_UNLIMITED),
    CYAML_FIELD_SEQUENCE("assembler_options",
                         CYAML_FLAG_POINTER | CYAML_FLAG_OPTIONAL,
                         yaml_profile, assembler_options,
                         &yaml_string_schema, 0, CYAML_UNLIMITED),
    CYAML_FIELD_SEQUENCE("assembler_args",
                         CYAML_FLAG_POINTER | CYAML_FLAG_OPTIONAL,
                         yaml_profile, assembler_args,
                         &yaml_string_schema, 0, CYAML_UNLIMITED),
    CYAML_FIELD_SEQUENCE("linker_options",
                         CYAML_FLAG_POINTER | CYAML_FLAG_OPTIONAL,
                         yaml_profile, linker_options,
                         &yaml_string_schema, 0, CYAML_UNLIMITED),
    CYAML_FIELD_SEQUENCE("linker_args",
                         CYAML_FLAG_POINTER | CYAML_FLAG_OPTIONAL,
                         yaml_profile, linker_args,
                         &yaml_string_schema, 0, CYAML_UNLIMITED),
    CYAML_FIELD_SEQUENCE("library_paths",
                         CYAML_FLAG_POINTER | CYAML_FLAG_OPTIONAL,
                         yaml_profile, library_paths,
                         &yaml_string_schema, 0, CYAML_UNLIMITED),
    CYAML_FIELD_SEQUENCE("libraries",
                         CYAML_FLAG_POINTER | CYAML_FLAG_OPTIONAL,
                         yaml_profile, libraries,
                         &yaml_string_schema, 0, CYAML_UNLIMITED),
...
static int profile_from_yaml(const char *path, cephyr_profile *profile,
                             char *error_message, size_t error_capacity)
{
    ...
    if ((raw->name && !profile_set_string(&profile->name, raw->name)) ||
        (raw->target && !profile_set_string(&profile->target_triple, raw->target)) ||
        (raw->opt_level && !profile_set_string(&profile->opt_level, raw->opt_level)) ||
        (raw->preprocessor && !profile_set_string(&profile->preprocessor, raw->preprocessor)) ||
        (raw->manifest_dir && !profile_set_string(&profile->manifest_dir, raw->manifest_dir)) ||
        (raw->sched_script && !profile_set_string(&profile->sched_script, raw->sched_script)) ||
        (raw->assembler && !profile_set_string(&profile->assembler, raw->assembler)) ||
        (raw->linker && !profile_set_string(&profile->linker, raw->linker))) {
        profile_error(error_message, error_capacity, "out of memory loading YAML profile");
        cyaml_free(&yaml_config, &yaml_profile_schema, raw, 0);
    ...
    for (uint64_t i = 0; i < raw->preprocessor_options_count; ++i)
        if (!profile_push_string(&profile->preprocessor_options,
                                 ...
                                 raw->preprocessor_options[i])) goto oom;
    for (uint64_t i = 0; i < raw->preprocessor_args_count; ++i)
        if (!profile_push_string(&profile->preprocessor_args,
                                 ...
                                 raw->preprocessor_args[i])) goto oom;
...
static int toml_copy_string(toml_table_t *table, const char *key, char **dst)
{
...
static int toml_copy_string_array(toml_table_t *root, const char *key,
...
static int profile_from_toml(const char *path, cephyr_profile *profile,
                             char *error_message, size_t error_capacity)
{
    FILE *file = NULL;
    ...
    if (integer.ok) profile->version = (int)integer.u.i;
    ...
    if (!toml_copy_string_array(root, "include_paths",
        ...
        !toml_copy_string_array(root, "linker_options",
                                ...
                                &profile->linker_option_count) ||
        !toml_copy_string_array(root, "linker_args",
                                &profile->linker_args,
                                &profile->linker_arg_count) ||
    ...
    }
    {
        ...
        if (value.ok) profile->pic = value.u.b;
        value = toml_bool_in(root, "pie");
        if (value.ok) profile->pie = value.u.b;
        value = toml_bool_in(root, "shared");
        if (value.ok) profile->shared = value.u.b;
    }
    array = toml_array_in(root, "kernels");
    for (int i = 0; array != NULL && i < toml_array_nelem(array); ++i) {
        toml_table_t *item = toml_table_at(array, i);
        toml_datum_t name = item ? toml_string_in(item, "name") : (toml_datum_t){0};
        toml_datum_t capability = item ? toml_string_in(item, "capability") : (toml_datum_t){0};
        toml_datum_t prefer = item ? toml_string_in(item, "prefer") : (toml_datum_t){0};
        if (item == NULL ||
            ...
            if (capability.ok) free(capability.u.s);
            if (prefer.ok) free(prefer.u.s);
            profile_error(error_message, error_capacity,
    ...
    for (int i = 0; array != NULL && i < toml_array_nelem(array); ++i) {
        toml_table_t *item = toml_table_at(array, i);
        toml_datum_t name = item ? toml_string_in(item, "name") : (toml_datum_t){0};
        toml_datum_t command = item ? toml_string_in(item, "command") : (toml_datum_t){0};
        if (item == NULL || !name.ok || !command.ok ||
            !profile_push_command(profile, name.u.s, command.u.s)) {
            if (name.ok) free(name.u.s);
            if (command.ok) free(command.u.s);
            profile_error(error_message, error_capacity,
                             ...
                             char *error_message, size_t error_capacity)
{
    static const char yaml[] =
        "# Cephyr profile v1.  Choose sched_script OR explicit kernels/rewrites.\n"
        "version: 1\n"
...
{
    if (profile == NULL || name == NULL) return NULL;
    for (size_t i = 0; i < profile->command_count; ++i)
