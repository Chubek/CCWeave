...
static void path_join(char *out, size_t capacity,
...
static void test_load_global_and_target_merge(void)
{
    char path[1024];
    char expected[1024];
    char error[256] = {0};
    ...
    assert(cephyr_stdlib_load(path, "x86_64-linux-gnu", &stdlib,
                              error, sizeof(error)));
...
static void test_load_other_target(void)
{
    char path[1024];
    char error[256] = {0};
    cephyr_stdlib stdlib;
    ...
    path_join(path, sizeof(path), CEPHYR_STDLIB_FIXTURE_DIR, "stdlib.yaml");
    memset(&stdlib, 0, sizeof(stdlib));
    assert(cephyr_stdlib_load(path, "riscv64-linux-gnu", &stdlib,
                              error, sizeof(error)));
    ...
    memset(&stdlib, 0, sizeof(stdlib));
    assert(cephyr_stdlib_load(path, "wasm32-wasi", &stdlib,
                              error, sizeof(error)));
    assert(stdlib.include_dir_count == 2);
    assert(stdlib.library_count == 1);
    assert(stdlib.start_file_count == 1);
...
static void test_load_failure(void)
{
    ...
    cephyr_stdlib stdlib;
    memset(&stdlib, 0, sizeof(stdlib));
    assert(!cephyr_stdlib_load("/nonexistent/stdlib.yaml", NULL,
                               &stdlib, error, sizeof(error)));
    assert(error[0] != '\0');
}

static void test_discovery_prefers_environment(void)
{
    char *manifest;
    int is_explicit = 0;
