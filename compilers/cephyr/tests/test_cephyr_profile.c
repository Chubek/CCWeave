...
static void test_yaml(void)
{
    ...
    path_join(path, sizeof(path), CEPHYR_PROFILE_FIXTURE_DIR,
              "profile.yaml");
...
static void test_init_and_discovery(void)
{
    ...
    assert(cephyr_profile_init_file(yaml_path, CEPHYR_PROFILE_YAML,
                                    error, sizeof(error)));
    memset(&profile, 0, sizeof(profile));
    ...
    assert(cephyr_profile_init_file(toml_path, CEPHYR_PROFILE_TOML,
                                    error, sizeof(error)));
