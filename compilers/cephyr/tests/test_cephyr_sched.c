...
static bool
has_edge (const char *text, unsigned from, unsigned to)
{
  char needle[64];
...
static void
check_pipeline (const char *script, const char *label)
{
  ...
  CCW_CHECK (ccw_plan_hash (first, first_hash), "%s must produce a hash",
             label);
  ...
  if (strcmp (label, "cephyr-o2") == 0)
    {
      ...
      for (char *line = strtok (node_copy, "\n"); line;
           line = strtok (NULL, "\n"))
        {
          ...
          if (sscanf (line, "node %u %u %255s", &id, &kind, name) != 3)
            continue;
          if (strcmp (name, "affine-extract") == 0)
            affine = id;
          else if (strcmp (name, "dep-poly") == 0)
            dependence = id;
          else if (strcmp (name, "isl-schedule") == 0)
            schedule = id;
          else if (strcmp (name, "tile-plan") == 0)
            tiling = id;
          else if (strcmp (name, "vec-width") == 0)
            vec_width = id;
          else if (strcmp (name, "loop-detect") == 0)
            loops = id;
          else if (strcmp (name, "vec-legality") == 0)
            vec_legality = id;
          else if (strcmp (name, "loop-vectorize") == 0)
            vec_plan = id;
          else if (strcmp (name, "vec-reduce") == 0)
            vec_reduce = id;
          else if (strcmp (name, "vec-lower-simde") == 0)
            vec_lower = id;
        }
      free (node_copy);
      ...
      CCW_CHECK (affine != 0 && dependence != 0 && schedule != 0 && tiling != 0
  ...
  char plan_path[256];
  snprintf (plan_path, sizeof (plan_path), "/tmp/ccw-%s-%ld.plan", label,
            (long)getpid ());
  error.code = 0;
  ...
  CCW_CHECK (ccw_plan_check (plan_path, CCW_MANIFEST_DIR, &error),
             "%s serialized plan must pass manifest validation: %s", label,
             error.message);
  unlink (plan_path);
