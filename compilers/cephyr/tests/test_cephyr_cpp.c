...
main (void)
{
  static const char source[] = "#define ANSWER 42\n"
                               "int answer(void) { /* keep the token stream "
                               "clean */ return ANSWER; }\n";
  cephyr_cpp_result result
      = cephyr_cpp_preprocess (source, strlen (source), "cpp-test.c", NULL, 0);

  CCW_CHECK (result.error_message == NULL, "preprocessing failed: %s",
  ...
  const cephyr_line_map_entry *mapping = cephyr_cpp_lookup_line (&result, 1);
  ...
  result = cephyr_cpp_preprocess_with_defines (
      defined_source, strlen (defined_source), "defined-test.c", NULL, 0,
      defines, 1);
  ...
  result = cephyr_cpp_preprocess_with_options (
      forwarded_source, strlen (forwarded_source), "forwarded-test.c", NULL, 0,
      NULL, 0, forwarded_options, 1, NULL, 0);
  CCW_CHECK (result.error_message == NULL,
  ...
  CCW_CHECK (
      error != NULL && strstr (error, "command") != NULL,
