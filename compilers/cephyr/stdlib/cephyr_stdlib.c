...
static const cyaml_schema_field_t yaml_stdlib_target_fields[] = {
  CYAML_FIELD_STRING_PTR ("triple", CYAML_FLAG_DEFAULT, yaml_stdlib_target,
                          triple, 0, CYAML_UNLIMITED),
  CYAML_FIELD_SEQUENCE ("include_dirs",
                        CYAML_FLAG_POINTER | CYAML_FLAG_OPTIONAL,
                        yaml_stdlib_target, include_dirs, &yaml_string_schema,
                        0, CYAML_UNLIMITED),
  CYAML_FIELD_SEQUENCE ("library_paths",
                        CYAML_FLAG_POINTER | CYAML_FLAG_OPTIONAL,
                        yaml_stdlib_target, library_paths, &yaml_string_schema,
                        0, CYAML_UNLIMITED),
  CYAML_FIELD_SEQUENCE ("libraries", CYAML_FLAG_POINTER | CYAML_FLAG_OPTIONAL,
                        yaml_stdlib_target, libraries, &yaml_string_schema, 0,
                        CYAML_UNLIMITED),
  CYAML_FIELD_SEQUENCE ("start_files",
                        CYAML_FLAG_POINTER | CYAML_FLAG_OPTIONAL,
                        yaml_stdlib_target, start_files, &yaml_string_schema,
                        0, CYAML_UNLIMITED),
  CYAML_FIELD_END
};
...
static const cyaml_schema_field_t yaml_stdlib_fields[] = {
  ...
  CYAML_FIELD_STRING_PTR ("name", CYAML_FLAG_OPTIONAL, yaml_stdlib, name, 0,
                          CYAML_UNLIMITED),
  CYAML_FIELD_STRING_PTR ("description", CYAML_FLAG_OPTIONAL, yaml_stdlib,
                          description, 0, CYAML_UNLIMITED),
  CYAML_FIELD_SEQUENCE ("include_dirs",
                        CYAML_FLAG_POINTER | CYAML_FLAG_OPTIONAL, yaml_stdlib,
                        include_dirs, &yaml_string_schema, 0, CYAML_UNLIMITED),
  CYAML_FIELD_SEQUENCE (
      "library_paths", CYAML_FLAG_POINTER | CYAML_FLAG_OPTIONAL, yaml_stdlib,
      library_paths, &yaml_string_schema, 0, CYAML_UNLIMITED),
  CYAML_FIELD_SEQUENCE ("libraries", CYAML_FLAG_POINTER | CYAML_FLAG_OPTIONAL,
                        yaml_stdlib, libraries, &yaml_string_schema, 0,
                        CYAML_UNLIMITED),
  CYAML_FIELD_SEQUENCE ("start_files",
                        CYAML_FLAG_POINTER | CYAML_FLAG_OPTIONAL, yaml_stdlib,
                        start_files, &yaml_string_schema, 0, CYAML_UNLIMITED),
  CYAML_FIELD_SEQUENCE ("targets", CYAML_FLAG_POINTER | CYAML_FLAG_OPTIONAL,
                        yaml_stdlib, targets, &yaml_stdlib_target_schema, 0,
                        CYAML_UNLIMITED),
  CYAML_FIELD_END
};

static const cyaml_schema_value_t yaml_stdlib_schema = {
  CYAML_VALUE_MAPPING (CYAML_FLAG_POINTER, yaml_stdlib, yaml_stdlib_fields),
};
...
static void
stdlib_error (char *out, size_t capacity, const char *fmt, ...)
{
  va_list ap;
...
static char *
stdlib_strdup (const char *value)
{
  kstring_t copy = { 0, 0, NULL };
  if (value == NULL || kputs (value, &copy) == EOF)
    return NULL;
  return ks_release (&copy);
}

static void
stdlib_free_list (char ***items, size_t *count)
{
...
static int
stdlib_push_owned (char ***items, size_t *count, char *owned)
{
  char **grown;
  if (owned == NULL)
    return 0;
...
static char *
stdlib_resolve_entry (const char *manifest_dir, const char *entry)
{
  kstring_t joined = { 0, 0, NULL };
  if (entry == NULL)
    return NULL;
  if (entry[0] == '/')
    return stdlib_strdup (entry);
...
static int
stdlib_push_entry (const char *manifest_dir, char ***items, size_t *count,
                   const char *entry)
{
...
static int
stdlib_push_verbatim (char ***items, size_t *count, const char *entry)
{
  return stdlib_push_owned (items, count, stdlib_strdup (entry));
}

static char *
stdlib_manifest_dirname (const char *path)
{
...
void
cephyr_stdlib_init (cephyr_stdlib *stdlib)
{
  if (stdlib == NULL)
    return;
...
void
cephyr_stdlib_destroy (cephyr_stdlib *stdlib)
{
  if (stdlib == NULL)
    return;
  free (stdlib->name);
...
static int
stdlib_merge_group (cephyr_stdlib *stdlib, const char *manifest_dir,
                    const char **include_dirs, uint64_t include_dir_count,
                    const char **library_paths, uint64_t library_path_count,
                    const char **libraries, uint64_t library_count,
                    const char **start_files, uint64_t start_file_count)
{
  for (uint64_t i = 0; i < include_dir_count; ++i)
    if (!stdlib_push_entry (manifest_dir, &stdlib->include_dirs,
                            &stdlib->include_dir_count, include_dirs[i]))
      return 0;
  for (uint64_t i = 0; i < library_path_count; ++i)
    if (!stdlib_push_entry (manifest_dir, &stdlib->library_paths,
                            &stdlib->library_path_count, library_paths[i]))
      return 0;
  for (uint64_t i = 0; i < library_count; ++i)
    if (!stdlib_push_verbatim (&stdlib->libraries, &stdlib->library_count,
                               libraries[i]))
      return 0;
...
int
cephyr_stdlib_load (const char *manifest_path, const char *target_triple,
                    ...
                    size_t error_capacity)
{
  ...
  if (raw == NULL)
    {
      stdlib_error (error_message, error_capacity, "stdlib manifest is empty");
      return 0;
    }
  ...
  if (raw->description != NULL)
    {
      ...
      if (stdlib->description == NULL)
        goto oom;
    }
  stdlib->manifest_path = stdlib_strdup (manifest_path);
  if (stdlib->manifest_path == NULL)
    goto oom;

  if (!stdlib_merge_group (
          stdlib, manifest_dir, raw->include_dirs, raw->include_dirs_count,
          raw->library_paths, raw->library_paths_count, raw->libraries,
          raw->libraries_count, raw->start_files, raw->start_files_count))
    goto oom;
      ...
      for (uint64_t i = 0; i < raw->targets_count; ++i)
        {
          ...
          if (target->triple == NULL
            ...
            continue;
          if (!stdlib_merge_group (
                  stdlib, manifest_dir, target->include_dirs,
                  target->include_dirs_count, target->library_paths,
                  target->library_paths_count, target->libraries,
                  target->libraries_count, target->start_files,
