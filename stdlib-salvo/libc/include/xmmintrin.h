...
static inline __m128 _mm_and_ps(__m128 a, __m128 b)
{
    __m128 r;
    for (int i = 0; i < 4; ++i) {
        union { float f; uint32_t u; } x = { a.v[i] }, y = { b.v[i] }, z;
        z.u = x.u & y.u; r.v[i] = z.f;
    }
    return r;
}
...
{
    __m128 r = {{a.v[imm8 & 3], a.v[(imm8 >> 2) & 3],
                 b.v[(imm8 >> 4) & 3], b.v[(imm8 >> 6) & 3]}};
    return r;
}
static inline float _mm_cvtss_f32(__m128 a) { return a.v[0]; }

static inline __m128d _mm_setzero_pd(void)
{
    __m128d r = {{0.0, 0.0}}; return r;
}
static inline __m128d _mm_set1_pd(double x)
{
    __m128d r = {{x, x}}; return r;
}
static inline __m128d _mm_set_pd(double e1, double e0)
{
    __m128d r = {{e0, e1}}; return r;
}
static inline __m128d _mm_loadu_pd(const double *p)
{
    __m128d r = {{p[0], p[1]}}; return r;
}
static inline __m128d _mm_load_pd(const double *p) { return _mm_loadu_pd(p); }
static inline void _mm_storeu_pd(double *p, __m128d a)
{
    p[0] = a.v[0]; p[1] = a.v[1];
}
static inline void _mm_store_pd(double *p, __m128d a) { _mm_storeu_pd(p, a); }

#define SALVO_M128D_BINOP(name, expr) \
    static inline __m128d name(__m128d a, __m128d b) { \
        __m128d r; for (int i = 0; i < 2; ++i) r.v[i] = (expr); return r; \
    }
