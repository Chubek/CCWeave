...
#define SALVO_M128I_BINOP(name, expr) \
    static inline __m128i name(__m128i a, __m128i b) { \
        __m128i r; for (int i = 0; i < 4; ++i) r.v[i] = (expr); return r; \
    }
...
{
    __m128i r; for (int i = 0; i < 4; ++i) r.v[i] = (~a.v[i]) & b.v[i]; return r;
}
static inline __m128i _mm_slli_epi32(__m128i a, int imm)
{
    __m128i r; for (int i = 0; i < 4; ++i) r.v[i] = (int32_t)((uint32_t)a.v[i] << imm); return r;
}
