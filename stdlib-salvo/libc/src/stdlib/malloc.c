...
static size_t salvo_align_up(size_t n)
{
    return (n + (SALVO_ALIGN - 1)) & ~(size_t)(SALVO_ALIGN - 1);
}

static salvo_block *salvo_heap_grow(size_t payload)
{
    size_t total = sizeof(salvo_block) + payload;
    salvo_block *block;

    if (total < payload) {          /* addition wrapped */
        errno = ENOMEM;
        return NULL;
    }
...
void *malloc(size_t size)
{
    ...
    for (block = salvo_heap_head; block != NULL; block = block->meta.next) {
        if (block->meta.free && block->meta.size >= size) {
            ...
            if (block->meta.size >=
                size + sizeof(salvo_block) + SALVO_ALIGN) {
                salvo_block *rest = (salvo_block *)
                    ((unsigned char *)(block + 1) + size);
                rest->meta.size =
                    block->meta.size - size - sizeof(salvo_block);
                rest->meta.free = 1;
