...
static void salvo_buf_emit(void *opaque, char c)
{
...
static void salvo_file_emit(void *opaque, char c)
{
...
static void salvo_emit_repeat(salvo_emit_fn emit, void *ctx, char c,
                              int count)
{
    while (count-- > 0)
        emit(ctx, c);
...
static void salvo_emit_string(salvo_emit_fn emit, void *ctx,
 ...
 * emitted. */
static int salvo_format_int(salvo_emit_fn emit, void *ctx,
...
static int salvo_vformat(salvo_emit_fn emit, void *ctx, const char *fmt,
                         va_list ap)
{
    int count = 0;
    ...
    for (const char *p = fmt; *p != '\0';) {
        ...
        if (*p == 'h') {
            ++p;
            if (*p == 'h') { ++p; spec.length = SALVO_LEN_HH; }
            else spec.length = SALVO_LEN_H;
        } else if (*p == 'l') {
            ++p;
            if (*p == 'l') { ++p; spec.length = SALVO_LEN_LL; }
            else spec.length = SALVO_LEN_L;
        } else if (*p == 'z') {
            ++p;
        ...
        case 'i': {
            ...
            switch (spec.length) {
            case SALVO_LEN_HH: sv = (signed char)va_arg(ap, int); break;
            case SALVO_LEN_H:  sv = (short)va_arg(ap, int); break;
            case SALVO_LEN_L:  sv = va_arg(ap, long); break;
            case SALVO_LEN_LL: sv = va_arg(ap, long long); break;
            case SALVO_LEN_Z:
            case SALVO_LEN_T:  sv = va_arg(ap, long); break;
            default:           sv = va_arg(ap, int); break;
            }
            negative = (sv < 0);
            magnitude = negative ? 0ULL - (unsigned long long)sv
                                 : (unsigned long long)sv;
            count += salvo_format_int(emit, ctx, magnitude, negative,
                                      10, 0, &spec);
            break;
        }
        case 'u':
        ...
        case 'o': {
            ...
            switch (spec.length) {
            case SALVO_LEN_HH: uv = (unsigned char)va_arg(ap, unsigned int); break;
            case SALVO_LEN_H:  uv = (unsigned short)va_arg(ap, unsigned int); break;
            case SALVO_LEN_L:  uv = va_arg(ap, unsigned long); break;
            case SALVO_LEN_LL: uv = va_arg(ap, unsigned long long); break;
            case SALVO_LEN_Z:
            case SALVO_LEN_T:  uv = va_arg(ap, unsigned long); break;
            default:           uv = va_arg(ap, unsigned int); break;
            }
            count += salvo_format_int(emit, ctx, uv, 0, base,
                                      conv == 'X', &spec);
            break;
        }
        case 'p': {
            ...
            pspec.plus = 0;
            pspec.space = 0;
            count += salvo_format_int(emit, ctx, (unsigned long long)
                                      (uintptr_t)ptr, 0, 16, 0, &pspec);
            break;
        }
        case 'c': {
